...
REM $Id: 4_0_add_mas_code_to_intrchg_pkg.sql 34 2017-10-06 20:14:33Z skumar $
...
DECLARE
    ...
    eff_date date := to_date('14-Oct-16', 'DD-MON-YY');
    ...
    cursor inst_fp is
        ...
        FROM fee_pkg
        WHERE institution_id IN ('101', '105', '107', '121') /* (, '106', '112', '113') */
...
BEGIN
    ...
    DBMS_OUTPUT.put_line('REM mas_codes added ' || rows_added );

    for ifp in inst_fp
    loop
        DBMS_OUTPUT.put_line(' ');
        DBMS_OUTPUT.put_line('REM in outer loop ' || ifp.institution_id || '  ' ||  ifp.fee_pkg_id);
