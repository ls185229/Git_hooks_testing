...
REM $Id: 3_0_update_template_mas_code_plsql.sql 38 2019-10-16 02:43:28Z bjones $

set serveroutput on;

declare
    cursor cur_usage is
        select distinct usage, card_scheme
        from flmgr_interchange_rates
        where card_scheme in ('02')
        and usage not in ('INTERCHANGE', 'DEFAULT_DISCOUNT')
        --and (region in ('CAN', 'ALL'))
        and (region in ('USA', 'ALL') or region is null)
        order by  usage, card_scheme;
...
--
        ...
        and (fir_l.region in ('USA', 'ALL') or fir_l.region is null)
        ---and (fir_l.region in ('CAN', 'ALL'))
        and fir_l.card_scheme is null
        --and fir_r.mas_desc is null
        ;
