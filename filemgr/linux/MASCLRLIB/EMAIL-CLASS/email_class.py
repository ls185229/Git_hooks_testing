...
A general purpose Python class for creating and sending emails.
    $Id: email_class.py 4590 2018-05-30 18:34:17Z lmendis $
"""
...
class EmailClass(object):
    ...
    +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
        Description:
            A general purpose email class for generating and sending email
            messages.
    ...
    __moduleName = 'email_class.py'  # Module name
    ...
    def PrintEmailParams(self):
        ...
        print(self.__formatBuffer.format(self.__errorMessage, \
                                self.__emailTextFile, \
                                self.__emailMessage, self.__emailTo, \
                                self.__emailToBuffer, self.__emailFrom, \
                                self.__emailFromBuffer, self.__emailCopy, \
                                self.__emailCcBuffer, self.__emailBlindCopy, \
                                self.__emailBccBuffer, self.__emailSubject, \
                                self.__emailBody, self.__emailConfig, \
                                self.__configSection, self.__emailFilesPath, \
                                self.__emailFilesList, \
                                self.__isEmailObjCreated))
    ...
    def ResetEmailParams(self, **keywordArgs):
        ...
        +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
            ResetEmailParams():
                Resets the email parameters based on the parameters passed to
                the function.
        ...
        try:
            # Make sure the email object is created before any files can
            # be attached
            if not self.__isEmailObjCreated:
                self.lineNum = self.GetLineNo() - 1
                self.__errorMessage = ( \
                        ('The SMPT email object has not been initialized. ' \
                        'Please create the email object prior to re-setting ' \
                        'email parameters. Error occurred in %s Module ' \
                        'ResetEmailParams() method at line #: %d.') \
            ...
            if keywordArgs:
                ...
                self.lineNum = self.GetLineNo() - 1
                self.__errorMessage = \
                        (('No data provided to reset the email parameters. ' \
                        'Error occurred in %s Module ResetEmailParams() ' \
                        'method at line #: %d.') \
                        % (self.__moduleName, self.lineNum))
                self.errorCode = 17
                self.__errorDesc_17 = \
                        ('Error occurred in %s module at Line #: %d. No ' \
                        'data provided to reset the email parameters.' \
                        % (self.__moduleName, self.lineNum))
                raise LocalException(self.__errorMessage)
    ...
    def AttachFiles(self, **keywordArgs):
        ...
        +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
            AttachFile():
                Checks the keyword parameters and sets up the directory path
                and the list of files to be attached to the email.

            Parameters:
                keywordArgs - Keyword arguments:
                                ATTACH_FILES_FOLDER - Folder containing the
                                                      files to attach.
        ...
        try:
            ...
            if not self.__isEmailObjCreated:
                ...
                self.__errorMessage = \
                        (('The SMPT email object has not been initialized. ' \
                        'Please create the email object prior to ' \
                        'attaching files. Error occurred in %s module ' \
            ...
            if keywordArgs:
                ...
                for keywordParam in keywordArgs:
                    ...
                    if keywordToUpper == 'ATTACH_FILES_FOLDER':
                        ...
                        # Make sure the the files to attach are passed as a
                        # list if type(keywordDict[keywordToUpper]) is not
                        # list:
                        if not isinstance(keywordDict[keywordToUpper], list):
                            self.lineNum = self.GetLineNo() - 1
                            self.__errorMessage = \
                                (('The files to attach must be a list. ' \
                                'Please ensure it is passed as a list. ' \
                                'Error occurred in %s module AttachFiles() ' \
                                'method at line #: %d.') \
                                % (self.__moduleName, self.lineNum))

                            self.errorCode = 15
                            self.__errorDesc_15 = \
                                ('Error occurred in %s module at Line #: ' \
                                '%d. The files to attach must be a list.' \
                                % (self.__moduleName, self.lineNum))
                            raise LocalException(self.__errorMessage)
                        else:
                ...
                self.lineNum = self.GetLineNo() - 1
                self.__errorMessage = \
                    (('Insufficient parameter values for attaching files. ' \
                    'Please check. Error occurred in %s Module ' \
                    'AttachFiles() method at line #: %d.') \
                    % (self.__moduleName, self.lineNum))
                self.errorCode = 14
                self.__errorDesc_14 = \
            ...
            if self.__emailFilesList:
                ...
                self.lineNum = self.GetLineNo() - 1
                print('    Email Files list is empty. Error occurred in ' \
                        '%s module AttachFiles() method at line #: %d.\n' \
                        % (self.__moduleName, self.lineNum))
            ...
            self.lineNum = self.GetLineNo() - 1
            self.__formatBuffer = ( \
                'Error occurred in {0} module AttachFiles() method ' \
                'at line #: {1}. Error: {2}')
            self.__errorMessage = \
            ...
            self.errorCode = 13
            self.__errorDesc_13 = \
                    ('Error occurred in %s module at Line #: %d. SMTP ' \
                    'Error: %s.' \
    ...
    def __ReadEmailTextFile(self):
        ...
        try:
            ...
            with open(self.__emailTextFile, 'r') as fileHandle:
                ...
                self.__emailMessage.attach( \
                                        MIMEText(fileHandle.read(), 'plain'))

        except IOError as exceptError:
            errorNum, errorString = exceptError.args
            self.lineNum = self.GetLineNo() - 2
            self.__formatBuffer = \
                    ('Error occurred in {0} Module __ReadEmailTextFile() ' \
                    'method at line #: {1}: I/O error reading file: ' \
                    '{2} ({3}): {4}')
            self.__errorMessage = \
                    self.__formatBuffer.format(self.__moduleName, \
                                self.lineNum, self.__emailTextFile, \
                                errorNum, errorString)
            self.errorCode = 12
            self.__errorDesc_12 = \
                    ('Error occurred in %s module at Line #: %d. ' \
                    'I/O error reading email text from file.' \
                    % (self.__moduleName, self.lineNum))
            return False
        except ValueError:
            self.lineNum = self.GetLineNo() - 1
            self.__formatBuffer = \
                    ('Error occurred in {0} Module __ReadEmailTextFile() ' \
                    'method at line #: {1}: Value error.')
            self.__errorMessage = \
                    self.__formatBuffer.format(self.__moduleName, self.lineNum)
            self.errorCode = 11
            self.__errorDesc_11 = \
                    ('Error occurred in %s module at Line #: %d. ' \
                    'Value error reading email text from file.' \
                    % (self.__moduleName, self.lineNum))
            return False
        except:
            self.lineNum = self.GetLineNo() - 1
            self.__formatBuffer = \
                    ('Error occurred in {0} Module __ReadEmailTextFile() ' \
                    'method at line #: {1}: Unexpected error: {2}')
            self.__errorMessage = \
                    self.__formatBuffer.format(self.__moduleName, \
                                        self.lineNum, sys.exc_info()[0])
            self.errorCode = 10
            self.__errorDesc_10 = \
                    ('Error occurred in %s module at Line #: %d. Unexpected ' \
                    'error reading email text from file.' \
                    % (self.__moduleName, self.lineNum))
            return False
        else:
            # Close the file.
    ...
    def SendEmail(self, resendMail=False):
        ...
        +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
            SendEmail():
                Constructs the email object and sends the mail to the
                recipients extracted from the email_config.ini file or as
                provided at the time of email object initialization.

            Parameters:
                resendMail - Flag to indicate if another email needs to be
                             sent. This is required to recreate the email
                             object because the smtp server gets closed
                             once an email is sent out.
        ...
        try:
            ...
            # If resending another email, make sure the smtp email object
            # is created. Otherwise, it would not work because the previous
            # email that was sent closes the smtp server when the
            # smtpServer.quit() is called in this function.
            ...
            if self.__emailTextFile:
                ...
                # This can be 'plain' or 'text' for the email body text to be
                # printed.
                self.lineNum = self.GetLineNo() + 1
                self.__emailMessage.attach(MIMEText(self.__emailBody, 'plain'))
                # self.__emailMessage.attach( \
                #                           MIMEText(self.__emailBody, 'text'))

            # Find the line number of the smpt call
            self.lineNum = self.GetLineNo() + 1
            ...
            self.__errorDesc_9 = \
                    ('Error occurred in %s module at Line #: %d. ' \
                    'Unable to read email text from file.' \
                    % (self.__moduleName, self.lineNum))
            return False
        except smtplib.SMTPException as exceptionError:
            self.__formatBuffer = \
                    ('Error occurred in {0} Module SendEmail() method at ' \
                    'line #: {1}. Error: {2}')
            self.__errorMessage = \
                self.__FormatMessage( \
                    self.__formatBuffer.format(self.__moduleName, \
    ...
    def __CheckEmailAddresses(self):
        ...
        +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
            __CheckEmailAddresses():
                Checks if the required as well as non required email addresses
                are present and builds the address strings to include single
            ...
            Returns:
                True if email addresses are validated and the email address
                strings are formatted accordingly. Flase else.
        +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
        """

        self.__formatBuffer = None

        # Now construct a string containing all the email to addresses.
        # The email to address is required to send email.
        if not self.__emailTo:
            self.lineNum = self.GetLineNo() - 1
            self.__formatBuffer = \
                    ("Email To: address is required to send emails. Please " \
                    "add it to the 'email_config.ini' file or provide it " \
                    "as a parameter to the email object at the time of " \
                    "initializing it and try again. Error occurred in " \
                    "module {0} __CheckEmailAddresses() method at line " \
                    "#: {1}")
            self.__errorMessage = \
                    self.__formatBuffer.format(self.__moduleName, self.lineNum)
            self.errorCode = 7
        ...
        # Now construct a string containing all the email from addresses.
        # The email from address is required to send email.
        if not self.__emailFrom:
            self.lineNum = self.GetLineNo() - 1
            self.__formatBuffer = \
                    ("Email From: address is required to send emails. " \
                    "Please add it to the 'email_config.ini' file or " \
                    "provide it as a parameter to the email object at " \
                    "the time of initializing it and try again. Error " \
                    "occurred in module {0} __CheckEmailAddresses() " \
                    "at line #: {1}")
            self.__errorMessage = \
                    self.__formatBuffer.format(self.__moduleName, self.lineNum)
            self.errorCode = 6
        ...
        # Now construct a string containing all the email copy addresses.
        # The email copy address is not required to send email.
        # if self.__emailCopy and type(self.__emailCopy) is list:
        ...
        # elif type(self.__emailCopy) is str:
        elif isinstance(self.__emailCopy, str):
            self.__emailCcBuffer = self.__emailCopy

        # print(self.__emailCcBuffer)

        # Now construct a string containing all the email blind copy
        # addresses. The email blind copy address is not required to
        # send email.
        # if self.__emailBlindCopy and type(self.__emailBlindCopy) is list:
    ...
    def __GetEmailConfigurations(self):
        ...
        +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
            __GetEmailConfigurations():
                Reads the email_config.ini file and gets the option values
                for each of the options under the section header provided
                during the email object initialization. Note that  if any
                keyword parameter values are passed at the time of the
                initialization, it is assumed the default values in the
                email config file will NOT override those values. This
                means that any keyword arguments passed to the email
                object supersedes the default values in the config file.

            Parameters:
                None.

            Returns:
                True if the email configurations were read correctly,
                False else.
        +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
        """
        ...
        try:
            ...
            if self.__emailConfig:
                ...
                configFile = os.path.join(basePath, 'email_config.ini')

            # print('ConfigPath: ', configFile, '\n')

            self.__formatBuffer = None

            # Create the ConfigParser object. Use RawConfigParser() with
            # allow_no_value set to True for section attributes with no value.
            if os.path.exists(configFile):
                ...
                if not sectionFound:
                    ...
                    self.__formatBuffer = \
                        ...
                        'arguments provided for initializing the email ' \
                        'object. Error occurred in {1} module ' \
                        '__GetEmailConfigurations() method at Line #: {2}')
                    ...
                    self.__errorDesc_5 = \
                            ('Error occurred in %s module at Line #: %d. ' \
                            'Unable to find the specified %s config section.' \
                            % (self.__moduleName, self.lineNum, \
                                self.__configSection))
                    raise LocalException(self.__errorMessage)
                ...
                for sectionOptions in configOptions:
                    ...
                    # Read and assign configuration values to the email class
                    # attributes. Note ast.leteral_eval() breaks up the leteral
                    # values in a multi value config string like: [SECTION]
                    ...
                    if optionToUpper == 'TO_ADDRESS':
                        ...
                        if not self.__emailFrom:
                            self.__emailFrom = \
                                        ast.literal_eval(emailFromAddress)
                    elif optionToUpper == 'CC_ADDRESS':
                        # Populate the Email CC: address
                        emailCopyAddress = \
                                configParserObj.get(self.__configSection, \
                                                    'CC_ADDRESS')
                        if not self.__emailCopy:
                            self.__emailCopy = \
                                    ast.literal_eval(emailCopyAddress)
                    elif optionToUpper == 'BCC_ADDRESS':
                        # Populate the Email BCC: address
                        emailBccAddress = \
                                configParserObj.get(self.__configSection, \
                                                    'BCC_ADDRESS')
                        if not self.__emailBlindCopy:
                            self.__emailBlindCopy = \
                                            ast.literal_eval(emailBccAddress)
                    elif optionToUpper == 'EMAIL_SUBJECT':
                        # Populate the Email Subject line
                        subjectText = \
                                configParserObj.get(self.__configSection, \
                                                    'EMAIL_SUBJECT')
                        if not self.__emailSubject:
                        ...
                        self.__formatBuffer = \
                                ...
                                "'email_config.ini' file. Please check the " \
                                "file option: {0}. Error occurred in {1} " \
                                "module __GetEmailConfigurations() " \
                                "method Line #:{2}")
                        print(self.__FormatMessage( \
    ...
    def __ReadArguments(self, **keywordArgs):
        ...
        ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
            ReadArguments():
                Reads the arguments passed and populates the class attributes
                for the email object.

            Parameters:
                keywordArgs - Keyword arguments:
                                READ_EMAIL_TEXT_FROM_FILE
                                    - Name of file to read the email text
                                      from.
                                READ_FROM_CONFIG_FILE
                                    - Name of file to read the email
                                      configurations.
                                CONFIG_SECTION_NAME
                                    - Configuration section name to read the
                                      email parameters.
                                EMAIL_BODY
                                    - Email body if a file containing a message
                                      is not provided.
                                TO_ADDRESS
        ...
        if keywordArgs:
            ...
            for keywordParam in keywordArgs:
                ...
                if keywordToUpper == 'READ_EMAIL_TEXT_FROM_FILE':
                    ...
                    self.__formatBuffer = None
                    self.__formatBuffer = \
                            ('Invalid keyword ({0}) passed to the ' \
                            '__ReadArguments() method in {1} at Line #: {2}')
                    print(self.__FormatMessage( \
                    ...
                    self.__errorDesc_3 = \
                            ('Error occurred in %s module at Line #: %d. ' \
                            'Invalid keywords passed to method.' \
                            % (self.__moduleName, self.lineNum))
    ...
    def __init__(self, **keywordArgs):
        ...
        +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
            __init__(self):
                Calls a function to initialize the EmailClass object
                attributes.
        ...
        try:
            # Extract the arguments passed at the time of object
            # initialization.
            if not self.__ReadArguments(**keywordArgs):
                self.lineNum = self.GetLineNo() - 1
                self.__errorMessage = \
                    ('Missing keyword arguments during the email object ' \
                    'initialization. Please check and provide the ' \
                    'required argrments. Error occurred in %s module ' \
                    '__init__() method at Line #: %d.' \
                     % (self.__moduleName, self.lineNum))
                self.errorCode = 2
                self.__errorDesc_2 = \
                    ('Error occurred in %s module __init__() method at Line ' \
                    '#: %d. Missing keyword arguments during the email ' \
                    'object initialization. ' \
                    % (self.__moduleName, self.lineNum))
                raise LocalException(self.__errorMessage)

            # Check if the email configurations are read from the
            # configuration file.
            if self.__emailConfig:
                self.lineNum = self.GetLineNo() - 1
                if not self.__GetEmailConfigurations():
                    self.__errorMessage = \
                        ('Unable to read the email configuration file. ' \
                        'Please ensure the file is configured properly. ' \
                        'Error occurred in %s module __init__() method at ' \
                        'Line #: %d.' % (self.__moduleName, self.lineNum))
                    self.errorCode = 1
                    self.__errorDesc_1 = \
                        ('Error occurred in %s module __init__() method at ' \
                        'Line #: %d. Unable to read the email configuration ' \
                        'file.' % (self.__moduleName, self.lineNum))
                    raise LocalException(self.__errorMessage)
            else:
                # Email configurations are not read from the config file, so
                # check the
            ...
            # Create email message object. Make it 'alternative' for multi part
            # email. For example you might want to attach a message body and
            # also files, so you need make the smtp email object multipart.
