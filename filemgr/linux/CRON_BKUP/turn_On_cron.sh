...
################################################################################
# $Id: turn_On_cron.sh 4420 2017-11-08 20:22:31Z skumar $
# $Rev: 4420 $
################################################################################
#
# File Name:  turn_On_cron.sh
#
# Description:  This script turns on CRON.
#              
...
init_script()
{
   ...
   archive_file_name=$locpth/ARCHIVE/turn_On_crontab_backup-$run_date$run_time.txt
   cron_txt_file_name=turnOn_cron.txt
   log_file=$locpth/LOG/turn_On_cron.log
...
then
   ...
   alert_someone "Run of $shell_script_name failed; CRON was not turned ON."
fi
