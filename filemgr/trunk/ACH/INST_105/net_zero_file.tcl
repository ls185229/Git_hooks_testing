	...
	"6" {incr LINE_COUNT
		#modify line for entity_id and payment seq nbr here
                if {[string range $line 1 2] == 22 || [string range $line 1 2] == 32} {
                  ...
                  set sum($gbin(IDENTIFIER)-cdt) [expr $sum($gbin(IDENTIFIER)-cdt) + [string trimleft [string range $line 29 38] 0]]
                } else {
                  #set sum([string range $line 39 44]-dbt) [expr $sum([string range $line 39 44]-dbt) + [string trimleft [string range $line 29 38] 0]]
                  set sum($gbin(IDENTIFIER)-dbt) [expr $sum($gbin(IDENTIFIER)-dbt) + [string trimleft [string range $line 29 38] 0]]
                }
		modify_detail $line
	    }
