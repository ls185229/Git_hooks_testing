...
locpth=/clearing/filemgr/ACH/INST_$inst_id/ARCHIVE
cd $locpth
...
print "Beginning FILE ENCRYPTION for JETPAY at `date +%Y%m%d%H%M%S`" | tee -a ../LOG/encrypt.log

if ./seek-encrypt-ach-file.tcl | tee -a ../LOG/encrypt.log; then 
	print "Encryption completed"
else
	echo "Encryption failed for ACH Files $mbody_l" | mailx -r $mailfromlist -s "$msubj_l" $mailtolist
fi

print "Ending TC57 FILE ENCRYPTION for JETPAY at `date +%Y%m%d%H%M%S`" | tee -a ../LOG/encrypt.log
