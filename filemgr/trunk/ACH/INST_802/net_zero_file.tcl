...
global SETTLE_DATE JULIAN_SETTLE_DATE DESC_DATE 802_trailer_line
...
#   set btch_hash [unoct [string range $line_in 10 19]]
   ...
   set a(orig_routing_num) [string range $line_in 79 86]
   set a(batch_num) [string range $line_in 87 93]
   set 802_trailer_line "[format_record a]"
    if {[catch {puts  $fileid "[format_record a]"} result ] } {
      puts $result "Error encountered writing File Header Record"
    }
...
proc create_offsetting_transactions_802 {line_in 802_line_in }  {
...
set data_sql "select unique IDENTIFIER from MASCLR.FLMGR_ACH_FIELD_VALUE_CTRL where institution_id = '$inst_id' and ach_cycle = '$cycl'"
   orasql $cursor $data_sql
   while {[set x [orafetch $cursor -dataarray gbin -indexbyname]] == 0 } {
    ...
    set ORIG_ROUTING_NUMBER "03191882"
    set ORIG_ID " 202948242"
    set batch_count [unoct [string range $line_in 1 6]]
    set block_count [unoct [string range $line_in 7 12]]
    set file_item_count [unoct [string range $line_in 13 20]]
    set TRACE_NO $file_item_count
    set hash_total [unoct [string range $802_line_in 10 19]]
    set debit_total [unoct [string range $802_line_in 20 31]]
    set credit_total [unoct [string range $802_line_in 32 43]]
...
#    incr file_item_count
    ...
    set a(item_cnt) $file_item_count
    set a(hash_num) [generate_hash_value $hash_total]
    set a(debit_total) [expr $batch_debit_total + $batch_debit_total]
    set a(credit_total) [expr $batch_credit_total + $batch_credit_total]
...
};# end create_offsetting_transactions_802
...
#             puts $line\r
	...
	"6" {incr LINE_COUNT
		#modify line for entity_id and payment seq nbr here
                if {[string range $line 1 2] == 22 || [string range $line 1 2] == 32} {
                  set sum([string range $line 39 44]-cdt) [expr $sum([string range $line 39 44]-cdt) + [string trimleft [string range $line 29 38] 0]]
                } else {
                  set sum([string range $line 39 44]-dbt) [expr $sum([string range $line 39 44]-dbt) + [string trimleft [string range $line 29 38] 0]]
                }
		modify_detail $line
	    }
		...
		if {$file_type == "52" && $inst_id == "802"} {
        		create_offsetting_transactions_802 $file_trailer $802_trailer_line
		} else {
