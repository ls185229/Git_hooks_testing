...
################################################################################
# $Id: new_122_ach_auto.sh 4573 2018-05-18 20:43:39Z bjones $
# $Rev: 4573 $
################################################################################
#
...
if [[ $psrun = "P" ]] ; then
    sht_name=$prdsn
    echo "Production run for $sht_name"
else
    sht_name=$tstsn
    echo "TEST run for $sht_name"
fi
...
if [ $1 ]; then
    ...
    if [ $1 -eq "52" ]; then
        ...
        echo "ACH BEGIN CYCLE $1 `date +%Y%m%d%H%M%S`" | tee -a $locpth/LOG/ach.log

        if $locpth/$code_nm | tee -a $locpth/LOG/ach.log; then
            echo "$locpth/$code_nm upload successful" | tee -a $locpth/LOG/ach.log
        else
            echo "$locpth/$code_nm failed" | mutt -s "$SYS_BOX Script $code_nm Failed" -- $MAIL_TO
        fi

        echo "<> END CYCLE $1 <> `date +%Y%m%d%H%M%S`" | tee -a $locpth/LOG/ach.log
