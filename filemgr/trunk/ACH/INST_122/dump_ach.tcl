...
################################################################################
# $Id: dump_ach.tcl 4569 2018-05-17 15:28:32Z bjones $
# $Rev: 4569 $
################################################################################
#
...
proc detail_rec {line_in}  {
    ...
    switch [string range $line_in 1 2] {
        ...
        "41" {puts "Return or NOC on a 42, 43 or 44 transaction"}
        "42" {puts "(Live GL account credit)"}
        "43" {puts "(Pre-note GL account credit)"}
        "46" {puts "Return or NOC on a 47, 48 or 49 transaction"}
        "47" {puts "(Live GL account debit)"}
        "48" {puts "(Pre-note GL account debit)"}
