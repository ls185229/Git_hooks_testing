...
switch $cycl {
	...
	"72"	{catch {set x [exec find . -name $inst_id-ACH_2_SUSQUEHANNA-72-$fdate* -mtime 0 ]} result; set seq $cycl}
...
switch $cycl {
        "62"   {catch {set x [exec find . -name $inst_id-ACH_2_FIRSTPREMIER-62-$fdate* -mtime 0 ]} result; set seq $cycl}
        "52"   {catch {set x [exec find . -name $inst_id-ACH_2_SUSQUEHANNA-52-$fdate* -mtime 0 ]} result; set seq "52"; set upld 0}
                default {puts "File type >$cycl< did not match criteria"; exit 1}
}
}
if {$inst_id == "811"} {
...
switch $cycl {
        "62"   {set seq $cycl}
        "72"   {set seq $cycl}
        "52"   {catch {set x [exec find . -name $inst_id-ACH_2_SUSQUEHANNA-52-$fdate* -mtime 0 ]} result; set seq "52" }
                default {puts "File type >$cycl< did not match criteria"; exit 1}
}
}
...
if {$dummy == 0} {
	set bank_ach_email ""
	switch $inst_id {
		"101" {set subj "101 $seq"}
                "105" {set subj "105 $seq";
			if {$seq == "52"} {
				set bank_ach_email ssach@susquehanna.net
			} else {
                                set bank_ach_email ""
			}
		      }
                "107" {set subj "107 $seq"}
                "811" {set subj "811 $seq"}
                "820" {set subj "820 $seq"}
		   default {set subj ""}
	}
