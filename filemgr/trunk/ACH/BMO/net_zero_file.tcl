...
if {[catch {set handler [oralogon masclr/masclr@$env(IST_DB)]} result]} {
   if { [catch { exec echo "DB connection error. netzero script failed to run" | mutt -s "ACH Netzero Script Error" -- Clearing.NCR PSTX@ncr.com } result] != 0 } {
...
if {$cycl != "002"} {
   ...
   if { $routing_nbr == ""} {
          if { [catch { exec echo "Routing Number is missing for the merchant: $entity_id" | mutt  \
                 -s "$env(SYS_BOX):::: Priority : Critical - Clearing and Settlement - ACH Netzero Script Error" -- Clearing.NCR PSTX@ncr.com } result] != 0 } {
           if { [string range $result 0 21] == "Waiting for fcntl lock" } {
              puts "Ignore mutt file control lock $result"
           } else {
              error "mutt error message: $result"
   ...
   if { $trans_amt == ""} {
          if { [catch { exec echo "Transaction Amount is missing for the merchant: $entity_id ,payment sequence number: $payment_seq_nbr"
                      | mutt -s "$env(SYS_BOX):::: Priority : Critical - Clearing and Settlement - ACH Netzero Script Error" -- Clearing.NCR PSTX@ncr.com } result] != 0 } {
           if { [string range $result 0 21] == "Waiting for fcntl lock" } {
              puts "Ignore mutt file control lock $result"
           } else {
              error "mutt error message: $result"
