...
if {[catch {set db_logon_handle [oralogon masclr/masclr@$env(IST_DB)]} result]} {
  exec echo "ACH Transmittal failed to run $0 DB logon" | mailx -r Clearing.JetPayTX@ncr.com -s "ACH Transmittal" Clearing.JetPayTX@ncr.com
  exit
}
...
set result 0
if {$result != 0} {
    exec echo "failed to insert into ACH Transmittal \n $result" | mailx -r Clearing.JetPayTX@ncr.com -s "Failed insert into ACH Transmittal" Clearing.JetPayTX@ncr.com
    exit 1
}
