...
################################################################################
# $Id: create_ptc_eft_129_file.tcl 4242 2017-07-25 14:08:04Z lmendis $
# $Rev: 4242 $
################################################################################
#
# File Name:  create_ptc_eft_129_file.tcl
#
...
proc getNextFileNumber {} {
  ...
  set query "SELECT seq_name, last_seq_nbr
          FROM seq_ctrl
          WHERE institution_id = '129' and seq_name in ('cr_file_create_nbr', 'dr_file_create_nbr') "
...
proc get_ptc_eft_credit_cad {} {
    ...
    #set output_file_name "/clearing/apps/mas/pdir/ositeroot/data/ach/DEFT-RPT859C-W_$temp_date"
    set output_file_name "$locpath/DEFT-RPT859C-W_$temp_date"
    ...
    puts -nonewline $output_file_id  "PTC859CR00"
    ...
    foreach line $data {
       ...
       if { ($var1  == $tran_code_ch) || ($var1 == $tran_code_sv )} {
            ...
            set sht_name "PTC"
            set sht_name [format "%-15s" $sht_name]
            ...
            set lng_name "PEOPLES TRUST COMPANY"
            ...
            puts -nonewline $output_file_id "1895859"    
...
proc get_ptc_eft_debit_cad {} {
    ...
    #set output_file_name "/clearing/apps/mas/pdir/ositeroot/data/ach/DEFT-RPT859D-W_$temp_date"
    set output_file_name "$locpath/DEFT-RPT859D-W_$temp_date"
    if {[catch {open $output_file_name {WRONLY CREAT EXCL} } output_file_id ]} {
        puts "Cannot open or create $output_file_name: $output_file_id"
        exit 1
    }
    ...
    # 03 Assigned Originator ID                                             11 - 20
    puts -nonewline $output_file_id  "PTC859DR00"
...
get_ptc_eft_credit_cad 

get_ptc_eft_debit_cad
