...
################################################################################
# $Id: fax_form.tcl 4249 2017-07-26 14:37:22Z bjones $
# $Rev: 4249 $
################################################################################
#
...
if {[catch {set db_logon_handle [oralogon masclr/masclr@$env(IST_DB)]} result]} {
  if { [catch { exec echo "ACH Transmittal failed to run" | mutt -s "ACH Transmittal" -- Clearing.JetPayTX@ncr.com } result] != 0 } {
