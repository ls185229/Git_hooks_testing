...
################################################################################
# $Id: net_zero_file.tcl 4378 2017-10-04 21:02:38Z bjones $
# $Rev: 4378 $
################################################################################
...
if {[catch {set handler [oralogon masclr/masclr@$env(IST_DB)]} result]} {
    if { [catch { exec echo "DB connection error. netzero script failed to run" | \
        mutt -s "ACH Netzero Script Error" -- Clearing.JetPayTX@ncr.com } result] != 0 } {
...
proc alt_ach_data {} {
    ...
    set data_sql "
        select INSTITUTION_ID, ACH_CYCLE, ACH_RECORD_TYPE, IDENTIFIER,
            FIELD_NAME, FIELD_VALUE
        from MASCLR.FLMGR_ACH_FIELD_VALUE_CTRL"
    orasql $cursor $data_sql
    while {[set x [orafetch $cursor -dataarray extnl_data -indexbyname]] == 0 } {
        set currkey  "$extnl_data(INSTITUTION_ID)-$extnl_data(ACH_CYCLE)-"
        append currkey "$extnl_data(ACH_RECORD_TYPE)-$extnl_data(IDENTIFIER)-"
        append currkey "$extnl_data(FIELD_NAME)"
        set alt_data($currkey) [string toupper $extnl_data(FIELD_VALUE)]
    }
}
...
proc modify_detail {line_in} {
    ...
    if {$cycl != "002"} {
        ...
        set payment_seq_nbr [unoct [string range $line_in 87 93]]
        ...
        set sql_string "
            ...
            from payment_log p, entity_acct e
...
proc modify_file_header {line_in} {
    global INSTITUTION_ID cursor inst_id cycl LINE_COUNT ORIG_NAME
    global ORIG_ROUTING_NUMBER ORIG_ID TRACE_NO DESTINATION_BANK T2_ACCOUNT alt_data
...
proc modify_batch_trailer {line_in} {
    ...
    global LINE_COUNT ORIG_NAME ORIG_ROUTING_NUMBER ORIG_ID TRACE_NO
    global DESTINATION_BANK T2_ACCOUNT
    ...
    if {$cycl == "002"} {
        ...
        set a(debit_total)  [expr [unoct [string range $line_in 20 31]] + \
                                  [unoct [string range $line_in 32 43]]]
        set a(credit_total) [expr [unoct [string range $line_in 20 31]] + \
                                  [unoct [string range $line_in 32 43]]]
    } else {
...
proc ach_create_offsetting_detail_transactions {line_in}  {
    ...
    global LINE_COUNT ORIG_NAME ORIG_ROUTING_NUMBER ORIG_ID TRACE_NO
    global DESTINATION_BANK T2_ACCOUNT ORIG_DESC
    global SETTLE_DATE JULIAN_SETTLE_DATE DESC_DATE

    set credit_total [unoct [string range $line_in 32 43]] ;
        # Debit total assigned to credit total
    set debit_total  [unoct [string range $line_in 20 31]] ;
        # Credit total assigned to debit total
...
proc create_offsetting_transactions {line_in}  {
    ...
    #    incr file_item_count
    incr batch_count
    set batch_debit_total  0 ;# Debits per batch for Batch Trailer Record
    set batch_credit_total 0 ;# Credits per batch for Batch Trailer Record
    set batch_entry_count  0 ;# Number of entrys per batch for Batch Trailer Record
    ...
    if {$credit_total > 0} {
        ...
        set batch_rdfi_hash [expr $batch_rdfi_hash + \
                                [unoct [string range $DESTINATION_BANK 0 7]]]
        set hash_total [expr  $hash_total + \
                                [unoct [string range $DESTINATION_BANK 0 7]]]
...
proc create_offsetting_transactions_105 {line_in}  {
    global fileid inst_id cycl alt_data sum
    global LINE_COUNT ORIG_NAME ORIG_ROUTING_NUMBER ORIG_ID TRACE_NO
    global DESTINATION_BANK T2_ACCOUNT ORIG_DESC
    global SETTLE_DATE JULIAN_SETTLE_DATE DESC_DATE
    global fileid inst_id cycl alt_data
    ...
    set data_sql "
        ...
        and ach_cycle = '$cycl'"
    orasql $cursor $data_sql
    while {[set x [orafetch $cursor -dataarray gbin -indexbyname]] == 0 } {
        lappend bin_list $gbin(IDENTIFIER)
    }

    set ORIG_NAME ""
    set ORIG_DESC ""
    set ORIG_ROUTING_NUMBER ""
    set ORIG_ID ""
    set DESTINATION_BANK ""
    set T2_ACCOUNT ""

    set ORIG_NAME "NCR PS"
    set ORIG_DESC "NCR PS"
    set ORIG_ROUTING_NUMBER "02607306"
    ...
    foreach bin $bin_list {
        ...
        set DESTINATION_BANK $alt_data($inst_id-$cycl-NET-$bin-DESTINATION_BANK)
        set T2_ACCOUNT $alt_data($inst_id-$cycl-NET-$bin-T2_ACCOUNT)
        ...
        if {$sum($bin-dbt) > 0} {
            ...
            set a(trace_num) "$tmp_str$tmp_str2"
            if {[catch {puts  $fileid "[format_record a]"} result ] } {
                $result "Error encountered writing zero net credit Detail Record"
            }
            incr LINE_COUNT
        }
...
};# end create_offsetting_transactions_105
...
alt_ach_data
if {(($file_type == "52") || ($file_type == "92")) && $inst_id == "105"} {
...
orasql $cursor $data_sql
while {[set x [orafetch $cursor -dataarray gbin -indexbyname]] == 0 } {
    set sum($gbin(IDENTIFIER)-cdt) 0
    set sum($gbin(IDENTIFIER)-dbt) 0
}
    ...
    switch [string range $line 0 0] {
        ...
        "6" {incr LINE_COUNT
            ...
            if {($code == "22") || ($code == "32") || ($code == "42")} {
                #set sum([string range $line 39 44]-cdt) \
                #        [expr $sum([string range $line 39 44]-cdt) + \
                #        [string trimleft [string range $line 29 38] 0]]
                set sum($gbin(IDENTIFIER)-cdt) \
                    [expr $sum($gbin(IDENTIFIER)-cdt) + \
                    [string trimleft [string range $line 29 38] 0]]
            } elseif {($code == "27") || ($code == "37") || ($code == "47")} {
                #set sum([string range $line 39 44]-dbt) \
                #        [expr $sum([string range $line 39 44]-dbt) + \
                #        [string trimleft [string range $line 29 38] 0]]
                set sum($gbin(IDENTIFIER)-dbt) \
                    [expr $sum($gbin(IDENTIFIER)-dbt) + \
                    [string trimleft [string range $line 29 38] 0]]
            } 
            modify_detail $line
        }
...
if {$cycl == "002"} {
    ...
    if {(($file_type == "52") || ($file_type == "92")) && $inst_id == "105"} {
        create_offsetting_transactions_105 $file_trailer
    } else {
