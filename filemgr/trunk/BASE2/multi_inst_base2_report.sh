...
################################################################################
# $Id: multi_inst_base2_report.sh 3723 2016-04-05 22:26:28Z millig $
# $Rev: 3723 $
################################################################################
#
...
if ./multi_inst_base2_report.tcl $1 >> $locpth/LOG/$logfile; then
	print "\nScript multi_inst_base2_report.tcl completed successfully" | tee -a $locpth/LOG/$logfile
	gzip ./EP747.txt
	mv ./EP747.txt.gz ./ARCHIVE/EP747.txt-$run_date.gz
else
	mbody="$SYS_BOX Script multi_inst_base2_report.tcl Failed"
