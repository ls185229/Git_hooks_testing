...
################################################################################
# $Id: multi_inst_base2_report.tcl 4910 2019-10-21 18:47:22Z smiller $
# $Rev: 4910 $
################################################################################
#
# File Name:  multi_inst_base2_report.tcl
#
...
get_cfg ./CFG
  ...
  catch {exec /clearing/filemgr/BASE2/recv_file_ftp.exp $logdate >> $logfile} err
...
foreach trknbr $trknbr_list {
  ...
  # send das (122) reports to esquire (121)
  if {$inst_id == "122"} {
      exec cp $zipfolder.zip ./INST_121/UPLOAD/ 
      cd ./INST_121/UPLOAD
  ...
  if { $inst_id != "101" && $inst_id != "107" && $inst_id != "127" && $inst_id != "128" && $inst_id != "129" && $inst_id != "130" && $inst_id != "134" } {
       catch {exec cp -f $zipfolder.zip ./INST_$inst_id/UPLOAD/ >> $logfile} err
       chk_err $err "cp -f $zipfolder.zip ./INST_$inst_id/UPLOAD/ >> $logfile"
...
foreach inst_id {101 107 117 121 127 128 129 130 134 122} {
  set filelist "" 
  set attachement_lst ""
  set locpth ""
