...
################################################################################
# $Id: ems_report.tcl 3597 2015-12-01 21:27:00Z bjones $
# $Rev: 3597 $
################################################################################
#
...
#               for dispute activity by institution.
#              
...
set sql1 "select e.EP_EVENT_ID,e.INSTITUTION_ID,e.TRANS_SEQ_NBR,e.TRANS_SEQ_NBR_ORIG,e.CARD_SCHEME,e.PAN,e.PAN_SEQ,e.ARN,e.SEND_ID,e.RECEIVE_ID,e.TRANS_DT,e.MERCH_ID,e.AMT_CH_BILL,e.CH_BILL_CCD,e.CH_BILL_EXP,e.AMT_TRANS,e.TRANS_CCD,e.TRANS_EXP,e.EMS_CASE_NBR,e.EMS_ITEM_TYPE,e.EMS_DUE_DATE,e.EMS_OPERATION,e.EMS_OPER_AMT_CR,e.EMS_OPER_ACCT_CR,e.EMS_OPER_AMT_DB,e.EMS_OPER_ACCT_DB,e.EMS_OPER_CCD,e.EMS_OPER_EXP,e.EMS_DEPARTMENT,e.PROCESS,e.REASON_CD,e.EVENT_STATUS,e.EVENT_REASON,e.MEMO_FREE_TEXT,e.EVENT_LOG_DATE,e.EVENT_UPD_DATE,e.CP_TO_HISTORY,e.EMS_SPEC_DATA1,e.EMS_SPEC_DATA2,e.EMS_SPEC_DATA3,e.EMS_SPEC_DATA4,e.IN_FILE_NBR,e.ISS_ACQ_IND,e.EMS_LEDGER, i.trans_seq_nbr, i.card_scheme, i.msg_type, i.in_file_nbr, i.in_batch_nbr, i.pan, i.amt_trans, i.cnv_rate_recon, i.trans_dt, i.function_cd, i.reason_cd, i.arn, i.auth_cd, i.merch_id, i.merch_name, i.settl_method_ind, i.msg_nbr, i.trans_clr_status, i.src_inst_id, i.src_file_type, i.activity_dt, i.msg_text_block  from EP_EVENT_LOG e, in_draft_main i where e.event_log_date = '$e_date' and e.EVENT_STATUS in ('NOTIFY', 'NEW') and e.trans_seq_nbr = i.trans_seq_nbr order by e.trans_seq_nbr"
puts $sql1
orasql $get2 $sql1
		...
		switch $a(EMS_ITEM_TYPE) {
			...
			"CR2" 	{if {$a(EVENT_REASON) == "EINC"} {
					...
					if {$a(EVENT_REASON) == "ERET"} {
						...
						if {$2rprej == 0} {
							exec echo "We have atleast one 2nd presentment rejected. Please, see EMS ALERT for Detail" | mailx -s "2ND PRESNTMENT REJECT ALERT" clearing@jetpay.com 
							set 2rprej 1
						}
					}} 
			"CB1" 	{set cb_type "CB1 ( 1st Chargeback )"; set fileid $fileid1; set enotify 0;
					set sql20 "select * from IN_DRAFT_MAIN where arn = '$a(ARN)' and (MSG_TYPE = '500' or MSG_TYPE = '600')"
					puts $sql20
					orasql $get20 $sql20
					...
					exec echo "[format %-30s $cb(MERCH_NAME)][format %-20s $cb(MERCH_ID)][format %-20s "Amount = $[expr $cb(AMT_TRANS)/100].[string range $cb(AMT_TRANS) [expr [string length $cb(AMT_TRANS)] - 2] end]"]\n" >> $tempeml
					}
			"CB2" 	{set cb_type "CB2 ( 2nd Chargeback )"; set fileid $fileid1; set enotify 0
					set sql21 "select * from IN_DRAFT_MAIN where arn = '$a(ARN)' and (MSG_TYPE = '500' or MSG_TYPE = '600')"
					puts $sql21
					orasql $get21 $sql21
					...
					exec echo "[string toupper $cb_type]\n[format %-30s $cb2(MERCH_NAME)][format %-20s $cb2(MERCH_ID)][format %-20s "Amount = $[expr $cb2(AMT_TRANS)/100].[string range $cb2(AMT_TRANS) [expr [string length $cb2(AMT_TRANS)] - 2] end]"]\n" >> $tempeml
				}
			"RR1" 	{set cb_type "RR1 ( Retrival Request )"; set fileid $fileid1; set enotify 0;
					set sql22 "select * from IN_DRAFT_MAIN where arn = '$a(ARN)' and (MSG_TYPE = '500' or MSG_TYPE = '600')"
					puts $sql22
					orasql $get22 $sql22
					...
					if {[string trim $a(CARD_SCHEME)] == "05"} {
						if {$a(REASON_CD) != "6802"} {
							exec echo "[format %-25s "Message Reason code"] : $a(REASON_CD)" >> $tempeml
							#SYANG exec echo "[format %-25s "Description"] : $msg_rsn_cd_desc($a(REASON_CD)-$a(FUNCTION_CD)-$a(MSG_TYPE))" >> $tempeml
							exec echo "[format %-25s "Description"] : Pls check MC DOC for DESC " >> $tempeml
						}
					} else {
						if {$a(REASON_CD) != "0"} {
							exec echo "[format %-25s "Message Reason code"] : $a(REASON_CD)" >> $tempeml
							#SYANG exec echo "[format %-25s "Description"] : $vs_msg_type_arry($a(REASON_CD))" >> $tempeml
							exec echo "[format %-25s "Description"] : Pls check VS DOC for DESC" >> $tempeml
						}
					}
					exec echo "[string toupper $cb_type]\n[format %-30s $rr1(MERCH_NAME)][format %-20s $rr1(MERCH_ID)][format %-20s "Amount = $[expr $rr1(AMT_TRANS)/100].[string range $rr1(AMT_TRANS) [expr [string length $rr1(AMT_TRANS)] - 2] end]"]\n" >> $tempeml
					}
...
if {[set sz [file size $outfile]] > 0 && [set sz [file size $outfile1]] > 0} {
	exec mutt -a $outfile -a $outfile1 -s "EP ALERTS" clearing@jetpay.com < body.txt
} elseif {[set sz [file size $outfile]] > 0 && [set sz [file size $outfile1]] == 0} {
	exec mutt -a $outfile -s "EP ALERTS" clearing@jetpay.com < body.txt
} elseif {[set sz [file size $outfile]] == 0 && [set sz [file size $outfile1]] > 0} {
	exec mutt -a $outfile1 -s "EP ALERTS" clearing@jetpay.com < body.txt
} else {
	exec echo "NO EP ALERTS" | mailx -r clearing@jetpay.com -s "NO EP ALERTS" clearing@jetpay.com 
}
...
if {$enotify == 0} {
	...
	foreach msgfile $eflist {
		...
		switch $mfile {
			"101.JPMS.email"    {catch {exec mutt -s "JPCS Chargeback Notifications For JPIS" clearing@jetpay.com merlinclearing@tsys.com < $mfile} result
								if {$result == ""} {
									exec rm $mfile
								}
								}
			"101.96.email"	    {catch {exec mutt -s "JPCS Chargeback Notifications For Iss 96" clearing@jetpay.com < $mfile} result
								if {$result == ""} {
									exec rm $mfile
								}
								}
			"101.85.email"	    {catch {exec mutt -s "JPCS Chargeback Notifications For Iss 85" clearing@jetpay.com < $mfile} result
								if {$result == ""} {
									exec rm $mfile
								}
								}
			"105.email"	    	{catch {exec mutt -s "JPCS Chargeback Notifications For JPIS" clearing@jetpay.com merlinclearing@tsys.com < $mfile} result
								if {$result == ""} {
									exec rm $mfile
								}                 
								}	
			"107.email"	    	{catch {exec mutt -s "JPCS Chargeback Notifications For JPIS" clearing@jetpay.com merlinclearing@tsys.com < $mfile} result
								if {$result == ""} {
									exec rm $mfile
								}                 
								}	
			"111.email"	    	{catch {exec mutt -s "JPCS Chargeback Notifications For REVWW" clearing@jetpay.com merlinclearing@tsys.com < $mfile} result
								if {$result == ""} {
									exec rm $mfile
								}                 
								}	
			"117.email"	    	{catch {exec mutt -s "JPCS Chargeback Notifications For SBC" clearing@jetpay.com merlinclearing@tsys.com < $mfile} result
								if {$result == ""} {
									exec rm $mfile
								}                 
								}	
			"121.email"	    	{catch {exec mutt -s "JPCS Chargeback Notifications For JPIS" clearing@jetpay.com merlinclearing@tsys.com < $mfile} result
								if {$result == ""} {
									exec rm $mfile
								}                 
								}	
			default     		{catch {exec mutt -s "JPCS Chargeback Notifications For [string range $mfile 0 2]" clearing@jetpay.com < $mfile} result
								 if {$result == ""} {
									exec rm $mfile
								 }
								}
...
if {$vsrej == 0} {
	...
	if {$result == ""} {
		catch {exec mutt -a $rejfile -s "BASEII REJECT ALERT" clearing@jetpay.com < mlbdy} result
		puts $result
...
if {$ipmrejectmail == 0} {
	catch {exec echo "We have ipm rejected. Please, see EMS ALERT for Detail" | mailx -s "IPM REJECT ALERT" clearing@jetpay.com} result
	puts $result
}
