...
# $Id: 105-ach-statement-w-time.tcl 2342 2013-07-06 00:38:37Z bjones $
...
#Fail message....
set fail_msg "105-ach-statement-w-time.tcl failed to run"

set SCRIPT_USER $env(SCRIPT_USER)
#Mode....
if { $box != "PD" } {
    set mode "TEST"
} else {
...
proc ach_report { institution_num } {
    ...
    set query_string_times "select unique to_char(payment_proc_dt, 'YYYY-MM-DD-HH24MISS') payment_time
                                 from ACCT_ACCUM_DET
    ...
    while {[orafetch $ach_cursor2 -dataarray times -indexbyname] != 1403} {
      ...
      set name_date $times(PAYMENT_TIME)
     ...
     where mtl.posting_entity_id = acq.entity_id
         ...
         and acq.institution_id =  '$institution_num'
         and mtl.SETTL_FLAG = 'Y'
         and mtl.PAYMENT_SEQ_NBR in (select PAYMENT_SEQ_NBR
                                 from ACCT_ACCUM_DET
                                 where PAYMENT_PROC_DT = to_date('$name_date', 'YYYY-MM-DD-HH24MISS')
                                  and institution_id = '$institution_num'
                                  and payment_cycle != '2'
                                  and payment_status = 'P'
                                 and entity_acct_id not in (select entity_acct_id
          ...
          if {$mode == "TEST"} {
              exec echo "Please see attached." | mutt -a "$cur_file_name" -s "$file_name" "$SCRIPT_USER@jetpay.com"
                ...
                exec echo "Please see attached." | \
                    mutt -a "$cur_file_name" -s "$file_name" \
                    "reports-clearing@jetpay.com,operations@jetpay.com,ACH-Reports@jetpay.com"
          }
    }
}
