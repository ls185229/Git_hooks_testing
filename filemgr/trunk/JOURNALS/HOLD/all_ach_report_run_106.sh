...
# $Id: all_ach_report_run_106.sh 1927 2012-12-13 15:18:13Z mitra $
...
if 106-ach-statement.tcl  >> $locpth/LOG/reports.log; then
   echo "106-ach-statement.tcl successfully\n" | tee -a $locpth/LOG/reports.log
else
   print "106-ach-statement.tcl run FAILED at `date +%Y%m%d%H%M%S`" | tee -a $locpth/LOG/reports.log
   tail -40 $locpth/LOG/reports.log  | mailx -r $MAIL_FROM -s "$SYS_BOX 106-ach-statement.tcl run Failed. Location: $locpth" $MAIL_TO
   exit 1
fi
