...
# $Id: 106-ach-statement.tcl 1927 2012-12-13 15:18:13Z mitra $ 

#=====================      MODIFICATION HISTORY        ========================
# Version g2.00 Sunny Yang 08-11-2010
...
proc ach_report { institution_num } {
     global  ach_cursor1 ach_cursor2 report_date name_date CUR_JULIAN_DATEX mode
      ...
      set GRAND_NETDEPOSIT 0
              ...
              set SUB_REPRESENTMENT_AMOUNT 0
              set SUB_REFUNDS      0
      ...
      if {$institution_num == "106"} {
            set query_string " select unique mtl.posting_entity_id as ENTITY_ID,
              ...
              sum(case when(mtl.tid_settl_method = 'C'  AND substr(mtl.tid, 1,6) in ('010042','010051')) then mtl.amt_original else 0 end) as MISC_ADJUSTMENTS_C,
              sum(case when(mtl.tid_settl_method <> 'C' AND substr(mtl.tid, 1,6) in ('010042','010051')) then mtl.amt_original else 0 end) as MISC_ADJUSTMENTS_D,
	       ...
	       and mtl.institution_id = acq.institution_id
               and mtl.institution_id = '$institution_num'
               and mtl.SETTL_FLAG = 'Y'
               and (mtl.institution_id, mtl.PAYMENT_SEQ_NBR) in (select institution_id, PAYMENT_SEQ_NBR
                                       from ACCT_ACCUM_DET
                                       where TRUNC(PAYMENT_PROC_DT, 'DDD') = '$report_date'
                                       ...
                                       and entity_acct_id not in (select entity_acct_id
                                                                    from entity_acct
                                                                     ...
                                                                     and stop_pay_flag = 'Y')) 
           group by mtl.posting_entity_id, acq.parent_entity_id, acq.entity_dba_name, acq.actual_start_date, mtl.PAYMENT_SEQ_NBR    
	...
	set e_bin "9999999"
      while {[orafetch $ach_cursor1 -dataarray t -indexbyname] != 1403} {                    
        ...
        if {[string range $t(ENTITY_ID) 0 5] != $e_bin && $e_bin != "9999999"} {
          set allout " ,,SUBTOTAL:$e_bin,[format "%0.2f" $SUB_SETTLED_BC],[format "%0.2f" $SUB_SETTLED_NONBC]"
          set allout "$allout,[format "%0.2f" $SUB_CHARGEBACK_AMOUNT],[format "%0.2f" $SUB_REPRESENTMENT_AMOUNT]"
          set allout "$allout,[format "%0.2f" $SUB_REFUNDS],[format "%0.2f" $SUB_MISC_ADJUSTMENTS]"
          set allout "$allout,[format "%0.2f" $SUB_ACH_RESUB],[format "%0.2f" $SUB_RESERVES]"
          puts $cur_file "$allout,[format "%0.2f" $SUB_DISCOUNT],[format "%0.2f" $SUB_NETDEPOSIT]\r"
              set SUB_SETTLED_BC  0
              set SUB_NETDEPOSIT  0
              set SUB_RESERVES    0
              set SUB_CHARGEBACK_AMOUNT    0
	  ...
	  puts $cur_file ",,,,,,,,"
              ...
              set SETTLED_BC           [ expr $t(SETTLED_C_BC) - $t(SPLIT_C) - $t(SALES_REVERSAL_D)]  ;# split fund
              ...
              set SUB_SETTLED_BC     	   [ expr $SUB_SETTLED_BC + $SETTLED_BC ]
              set SUB_NETDEPOSIT           [ expr $SUB_NETDEPOSIT +  $NETDEPOSIT ]
              set SUB_RESERVES             [ expr $SUB_RESERVES + $RESERVES ]
              set SUB_CHARGEBACK_AMOUNT    [ expr $SUB_CHARGEBACK_AMOUNT + $CHARGEBACK_AMOUNT ]
              set SUB_REPRESENTMENT_AMOUNT [ expr $SUB_REPRESENTMENT_AMOUNT + $REPRESENTMENT_AMOUNT ]
              set SUB_REFUNDS              [ expr $SUB_REFUNDS + $REFUNDS ]
              set SUB_SETTLED_NONBC        [ expr $SUB_SETTLED_NONBC + $SETTLED_NONBC ]
              set SUB_MISC_ADJUSTMENTS     [ expr $SUB_MISC_ADJUSTMENTS + $MISC_ADJUSTMENTS ]
              set SUB_DISCOUNT             [ expr $SUB_DISCOUNT + $DISCOUNT ]
              set SUB_ACH_RESUB            [ expr $SUB_ACH_RESUB + $ACH_RESUB ]
	...
	set e_bin [string range $t(ENTITY_ID) 0 5]
