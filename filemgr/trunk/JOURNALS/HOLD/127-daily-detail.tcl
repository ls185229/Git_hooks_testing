...
################################################################################
# $Id: 127-daily-detail.tcl 2712 2014-10-16 20:16:43Z mitra $
# $Rev: 2712 $
################################################################################
...
if {[catch {set handler [oralogon masclr/masclr@$clrdb]} result]} {
    puts ">>>$result<<<"
    exec echo $fail_mag | mutt -s $fail_mag $mail_err
    exit 1
}
...
proc do_report {instID} {
    global daily_cursor mode mail_to
...
#===============================================================================
    ...
    if {$mode == "PROD"} {
        set upath "[pwd]/INST_127/UPLOAD"
        set apath "[pwd]/INST_127/ARCHIVE"
        set cur_file_name     "./127.DAILY.DETAIL.REPORT.$CUR_JULIAN_DATEX.$filedate.001.csv"
        set file_name         "127.DAILY.DETAIL.REPORT.$CUR_JULIAN_DATEX.$filedate.001.csv"
        set cur_file          [open "$cur_file_name" w]
    } elseif {$mode == "TEST"} {
        set upath "./UPLOAD"
        set apath "./ARCHIVE"   
        set cur_file_name     "./127.DAILY.DETAIL.REPORT.$CUR_JULIAN_DATEX.$filedate.TEST.csv"
        set file_name         "127.DAILY.DETAIL.REPORT.$CUR_JULIAN_DATEX.$filedate.TEST.csv"
        set cur_file          [open "$cur_file_name" w]
    }
