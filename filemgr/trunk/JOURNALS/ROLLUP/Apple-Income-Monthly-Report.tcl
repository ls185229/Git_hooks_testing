...
# $Id: Apple-Income-Monthly-Report.tcl 2298 2013-05-16 17:02:19Z mitra $

#***********************************************************************************
#
# -  The script simply runs from the 1st day of the last month to the last day. 
#
# -  This will run into issues when the last day is on a weekend. You need to 
#    manually change the dates before runing this report. 
#    
# -  Run:  tclsh Apple-Income-Monthly-Report.tcl &
#
# -  This automatically emails the generated file to Reports-clearing@jetpay.com 
#    and accounting@jetpay.com
# 
#***********************************************************************************
...
set message "Apple Income Monthly Report failed to run."
...
if {[catch {set handlerM [oralogon masclr/masclr@$clrdb]} result]} {
     exec echo $message | mutt -s $message -- $mailto_error  
     exit
}
...
puts $cur_file "Merchant ID, Merchant Name, Tran Count, Sales, Gross Income, Interchange,\
                Assess Fee, MC&VS Fees, Discount,  Net Income\r\n"
...
set query_fee "
    SELECT unique entity_id,  
        SUM(CASE WHEN( m.tid_settl_method = 'C') 
                 then AMT_ORIGINAL else 0 end) as TOTAL_FEES_C,
        SUM(CASE WHEN( m.tid_settl_method <> 'C') 
                 then AMT_ORIGINAL else 0 end) as TOTAL_FEES_D,
        SUM(CASE WHEN(mas_code in ('MC_BORDER','MC_GLOBAL_ACQ') and tid like '010004%') 
                 then amt_original else 0 end) as MC_BORDER_FEE,
        SUM(CASE WHEN(mas_code in ('VS_ISA_FEE','VS_ISA_HI_RISK_FEE') and tid like '010004%') 
                 then amt_original else 0 end) as VS_ISA_FEE, 
	SUM(CASE WHEN( m.tid in ('010004020000', '010004020005') and m.tid_settl_method = 'D' ) 
                 then AMT_ORIGINAL else 0 end) as INTERCHANGE_AMT_D,
	SUM(CASE WHEN( m.tid in ('010004020000', '010004020005') and m.tid_settl_method = 'C' ) 
                 then AMT_ORIGINAL else 0 end) as INTERCHANGE_AMT_C,
	SUM(CASE WHEN(mas_code in ('VISA_DISCOUNT', 'MC_DISCOUNT')) 
                 then m.amt_original else 0 end ) as DISCOUNT,
    ...
    WHERE m.PAYMENT_SEQ_NBR in (select PAYMENT_SEQ_NBR
                                from ACCT_ACCUM_DET
                                where PAYMENT_PROC_DT >= '01-$report_my' AND PAYMENT_PROC_DT < '01-$next_my'
    ...
    and tid <> '010014020005'
    GROUP by entity_id 
    ORDER by entity_id"
                       
puts "query_fee -$query_fee"               

orasql $M_cursor $query_fee

while {[set test [orafetch $M_cursor -dataarray fee -indexbyname]] == 0} { 
...
set query_sale "
    SELECT unique entity_id, 
        (select unique entity_dba_name from acq_entity where entity_id = m.entity_id) as entity_name, 
             SUM(CASE WHEN (m.tid in ('010003005102','010003005101','010003005201','010003005202','010123005101',\
                                      '010123005102','010003005204') and m.CARD_SCHEME in ('04','05')) 
                      Then 1 else 0 end ) AS SALES_COUNT ,
	     (SUM(CASE WHEN (m.tid in ('010003005102','010003005101','010003005201','010003005202','010123005101',\
                                       '010123005102','010003005204') and m.CARD_SCHEME in ('04','05') 
                                        and m.tid_settl_method = 'C' ) 
                      Then m.amt_original else 0 end ) \
    	    - SUM(CASE WHEN (m.tid in ('010003005102','010003005101','010003005201','010003005202','010123005101',\
                                       '010123005102','010003005204') and m.CARD_SCHEME in('04','05') 
                                        and m.tid_settl_method <> 'C' ) 
                      Then m.amt_original else 0 end ) )AS SALES_AMT
    FROM mas_trans_log m
    WHERE gl_date >= '01-$report_my' AND gl_date < '01-$next_my' 
    and institution_id = '101' 
    and entity_id in ('447474210000133', '447474210000135', '447474210000136', '447474210000139', '447474310000101')
    group by entity_id 
    order by entity_id " 
...
while {[set test [orafetch $M_cursor -dataarray sale -indexbyname]] == 0} {
        ...
        puts $cur_file " $MERCHANT_NUM,$MERCHANT_NAME, $TRAN_COUNT,$SALES_AMT,$GROSS_INCOME,\
                         $INTERCHANGE_AMT,$ASSESSMENT_AMT,$MC_VS_FEE,$DISCOUNT, $NET \r"
...
puts $cur_file "TOTAL: , , $SUB_TRAN_COUNT, $SUB_SALES_AMT, $SUB_GROSS, $SUB_INCHG, $SUB_ASSESS,\
                $SUB_FEE, $SUB_DISCOUNT, $SUB_NET\r"

close $cur_file

exec echo "Please see attached." | mutt -a $cur_file_name -s $file_name $mailto 
