...
puts "today: $today...$tomorrow: $tomorrow"
      ...
      set cur_file_name "./106.440339.REPORT.HEADER.$CUR_JULIAN_DATEX.$name_date.001.csv"
      set file_name     "106.440339.REPORT.HEADER.$CUR_JULIAN_DATEX.$name_date.001.csv"
      set cur_file      [open "$cur_file_name" w]
      puts $cur_file "ISO,106-440339"
      puts $cur_file " "
...
#*** discover id ***
      orasql $header_cursor11 "select unique visa_id, discover_id
                                from merchant
                               where substr(visa_id,1,6) in ( '440339')"
...
#*** Terminal Info ***
      set query_terminal "select  unique m.visa_id as VID, count(t.tid) as TERM_CNT
                           ...
                           where t.mid = m.mid and
                                 substr(m.visa_id, 1, 6) in ('440339') and 
                                 t.active = 'A'
                           group by m.visa_id
                           order by m.visa_id"
      puts $query_terminal
...
#===============================================================================
     ...
     puts "MAIN: $qry_main"
      ...
      exec uuencode $cur_file_name $file_name | mailx -r clearing@jetpay.com -s $file_name clearing@jetpay.com cschang@jetpay.com 
