...
################################################################################
# $Id: daily_detail_settled_v2.tcl 4843 2019-04-19 14:48:36Z skumar $
# $Rev: 4843 $
################################################################################
#
#    File Name   -  daily_detail_settled_v2.tcl
#
...
proc readCfgFile {} {
   ...
   if { [info exists cfg_file_name] } {
         puts  " Config file argument: $cfg_file_name"
          ...
          set cfg_file_name daily_detail_settled_v2.cfg
...
proc getRejects {dateID} {
    ...
    set rejectsQuery "select count(*) as count,
                      ...
                      where in_draft_main.card_scheme = card_scheme.card_scheme AND
                        ...
                        in_draft_main.in_file_nbr in (select in_file_nbr from in_file_log
                                                      where trunc(incoming_dt) between to_date($reportStartDate,'YYYYMMDD') and
                                                                                        to_date($reportEndDate,'YYYYMMDD') AND
    ...
    MASCLR::log_message "Rejects Query: $rejectsQuery" 3
...
proc getChargebacksFromClearing {dateID} {
    ...
    set chargebackQuery "
        ...
        where (idm.sec_dest_inst_id = '$sql_inst' OR 
              ...
              trunc(incoming_dt) between to_date($reportStartDate,'YYYYMMDD') and to_date($reportEndDate,'YYYYMMDD')
              and ifl.institution_id in ('02', '04','05','03','08')
    ...
    MASCLR::log_message "Chargebacks Query: $chargebackQuery" 3
...
where in_draft_main.card_scheme = card_scheme.card_scheme AND
      ...
      where 
      trunc(incoming_dt) between to_date($reportStartDate,'YYYYMMDD') and to_date($reportEndDate,'YYYYMMDD') 
      and institution_id in ('02', '04','05','03','08'))
...
order by substr(in_draft_main.sec_dest_inst_id,1,4),
    ...
    MASCLR::log_message  "Arbitrations Query(Clearing - $dateID): $arbitrationQuery" 3
...
proc getChargebacksFromMAS {dateID} {
    ...
    set chargebackQuery "
        ...
        WHERE mfl.INSTITUTION_ID = '$sql_inst'
            ...
            AND trunc(mfl.receive_date_time)  between to_date($masStartDate,'YYYYMMDD') and to_date($masEndDate,'YYYYMMDD')
    ...
    MASCLR::log_message  "Chargebacks Query - MAS : $chargebackQuery" 3
    orasql $daily_cursor $chargebackQuery
...
proc getArbitrationsFromMAS {dateID} {
    ...
    MASCLR::log_message "Arbitrations Query - MAS($dateID): $arbitrationQuery" 3
...
proc getDbtByNetwork {} {
    ...
    MASCLR::log_message  "DBT Network Query: $dbtNetworkQuery" 3
...
if { [info exists arg_cycle] } {
      if { $arg_cycle == 5 } {
           set file_name  "$sql_inst.Daily_Detail_Report_V2_nxdy.$filedate.csv"
      } else {
           set file_name  "$sql_inst.Daily_Detail_Report_V2_Cycle.$arg_cycle.$filedate.csv"
      }
} else {
       set file_name  "$sql_inst.Daily_Detail_Report_V2.$filedate.csv"
}
