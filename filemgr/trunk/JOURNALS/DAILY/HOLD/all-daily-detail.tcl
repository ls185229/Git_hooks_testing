...
################################################################################
# $Id: all-daily-detail.tcl 4054 2017-02-06 23:16:14Z millig $
# $Rev: 4054 $
################################################################################
#
...
if {[catch {set handler [oralogon masclr/masclr@$clrdb]} result]} {
    puts ">>>$result<<<"
    exec echo "daily-detail.tcl failed to run" | mutt -s "daily-detail.tcl failed to run" -- clearing@jetpay.com
    exit 1
}
...
###
    ...
    WHERE institution_id in ($sql_inst)
      ...
      AND (entity_id NOT in $JPC_LST OR institution_id = '121')
      AND settl_flag = 'Y'
...
###
    ...
    SELECT TO_CHAR( aad.PAYMENT_PROC_DT, 'DD') dd,
      ...
      sum(CASE WHEN(mtl.card_scheme = '04' and mtl.TID_SETTL_METHOD = 'C' and TID in ('010004160000', '010004160001'))
               then AMT_ORIGINAL else  0 end) as CHR_VS_C_FEE,
      sum(CASE WHEN(mtl.card_scheme = '04' and mtl.TID_SETTL_METHOD <> 'C' and TID in ('010004160000', '010004160001'))
               then AMT_ORIGINAL else  0 end) as CHR_VS_D_FEE,

      sum(CASE WHEN(mtl.card_scheme = '05' and mtl.TID_SETTL_METHOD = 'C' and TID in ('010004160000', '010004160001'))
               then AMT_ORIGINAL else  0 end) as CHR_MC_C_FEE,
      sum(CASE WHEN(mtl.card_scheme = '05' and mtl.TID_SETTL_METHOD <> 'C' and TID in ('010004160000', '010004160001'))
               then AMT_ORIGINAL else  0 end) as CHR_MC_D_FEE,

      sum(CASE WHEN(mtl.card_scheme = '03' and mtl.TID_SETTL_METHOD = 'C' and TID in ('010004160000', '010004160001'))
               then AMT_ORIGINAL else  0 end) as CHR_AX_C_FEE,
      sum(CASE WHEN(mtl.card_scheme = '03' and mtl.TID_SETTL_METHOD <> 'C' and TID in ('010004160000', '010004160001'))
               then AMT_ORIGINAL else  0 end) as CHR_AX_D_FEE,

      sum(CASE WHEN(mtl.card_scheme = '08' and mtl.TID_SETTL_METHOD = 'C' and TID in ('010004160000', '010004160001'))
    ...
    WHERE (mtl.PAYMENT_SEQ_NBR = aad.PAYMENT_SEQ_NBR AND mtl.institution_id = aad.institution_id)
       ...
       AND (mtl.entity_id NOT in $JPC_LST OR mtl.institution_id = '121')
...
###
    ...
    WHERE (m.payment_seq_nbr = aad.payment_seq_nbr AND m.institution_id = aad.institution_id)
      ...
      AND (m.entity_id NOT in $JPC_LST OR m.institution_id = '121') 
...
#*******    Pull the reject counts and amounts  *******#
    ...
    set query_string "                       
        ...
        where t.tid in ('010103005101', '010103005102', '010103005201', '010103005202')
          ...
          and (idm.merch_id NOT in $JPC_LST OR idm.SEC_DEST_INST_ID = '121')
