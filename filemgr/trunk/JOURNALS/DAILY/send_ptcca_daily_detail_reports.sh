...
################################################################################
# $Id: send_ptcca_daily_detail_reports.sh 4217 2017-06-26 17:39:20Z bjones $
# $Rev: 4217 $
################################################################################
#
# File Name:  send_b2bca_daily_detail_reports.sh
#
# Description:  This script transmits Daily Detail reports to B2B and PTC.
#
...
init_script()
{
   ...
   log_file=$locpth/LOG/send_ptcca_daily_detail_reports.log
   dd_report_mask=129.Daily_Detail_Report.$run_date*
   echo $dd_report_mask
...
#-- Upload to B2Billing
#--
echo "Attempting to upload Daily Detail report to B2Billing at `date +%H`:`date +%M`:`date +%S`"
/usr/bin/sftp b2payments@sftp.jetpay.com << EOF
cd reports
put $ftp_ach_src_dir/$dd_report_mask
quit
EOF
transfer_result=$?
...
then
    alert_someone "Daily Detail report file transfer to B2Billing $0 processing for run date $file_run_date failed."
fi
...
#--
echo "Attempting to upload Daily Detail report to Peoples Trust at `date +%H`:`date +%M`:`date +%S`"
...
if [ $transfer_result -ne 0 ]
then
    alert_someone "Daily Detail report file transfer to Peoples Trust $0 processing for run date $file_run_date failed."
fi
print "___________________________________________________________________________\n" >> $log_file
