...
#$Id: amex_tilr.tcl 4734 2018-10-09 12:53:47Z skumar $
#Script arguements :
#  Date - Date of the amex TILR file
#  Mode - PROD or TEST
#  mail_id - mail id the reports to be sent
# for eg : amex_tilr.tcl 20181006 TEST username@jetpay.com
...
proc parse_Sum_Det_HDr_Rec {rec} {
    ...
    MASCLR::log_message "parse_Sum_Det_HDr_Rec - CURR_SE_NUM: $curr_se_num" 3
if { $curr_se_num != 2423794213 } { 
    set amex_se_map_query "select distinct(institution_id)
    ...
    if {[orafetch $core_amex_se_map_c -dataarray core_amex_se_map_orig -indexbyname] != 1403 } {
       set institution_id        "$core_amex_se_map_orig(INSTITUTION_ID)"
        ...
        MASCLR::log_message "the institution ID not found for se_number $curr_se_num"
        set institution_id "000"
        #exit 1
    }
    ...
    if { $institution_id != "000"} {
        dict set dict_acquirer $curr_acq name $instId_name
    } else {
        dict set dict_acquirer "0000000000" name $instId_name
    }
...
if { [lindex $argv 0] == "" } {
    ...
    set run_file_date [lindex $argv 0]
    puts "using date $run_file_date"
    set run_file_time "-[clock format [clock seconds] -format "%Y%m%d"]-[clock format [clock seconds] -format "%H%M%S"]"
}
...
puts "mode is $mode"
global report_mail_id
set report_mail_id ""

if {$mode == "PROD"} {
    set inpath "/clearing/filemgr/AMEX/SETTLEMENT/TILR/ARCHIVE/"
    set apath "/clearing/filemgr/JOURNALS/AMEX_TILR/ARCHIVE/"
    set cnfig_source "./CONFIG/amex_config.tc"
} else {
    set inpath "/clearing/filemgr/AMEX/SETTLEMENT/TILR/ARCHIVE/"
    set apath "/clearing/filemgr/JOURNALS/AMEX_TILR/TEST_ARCHIVE/"
    set cnfig_source "./CONFIG/amex_test_config.tc"
    set report_mail_id [lindex $argv 2]
    puts "In TEST mode , report_mail_id is $report_mail_id"
}
...
# set tilr_infiles [glob -nocomplain ${inpath}JETPAYPRD.EPTRN*.[clock format [clock seconds] -format %Y%m%d]]
set tilr_infiles [glob -nocomplain ${inpath}JETPAYPRD.EPTRN*.$run_file_date]
if {$tilr_infiles == ""} {
    # puts "$argv0 did not find the input file matching: ${inpath}AMEX_TILR.[clock format [clock seconds] -format %Y%m%d]"
    puts "$argv0 did not find the input file matching: ${inpath}AMEX_TILR.$run_file_date"
	MASCLR::mutt_send_mail "clearing@jetpay.com,reports-clearing@jetpay.com"  "$argv0 - File not found" "$argv0 could not find today's AMEX_TILR file." 
    exit 1
}
...
###
if { [regexp -- {JETPAYPRD\.EPTRN.{15,15}\.([0-9]{08,08})} $infile_name dummy1 parsed_infile_dt] } {
        puts "Date parsed from infile name: $parsed_infile_dt"
} else {
    puts "Error parsing date from infile.. $argv0 did not find an input file matching format: JETPAYPRD.EPTRN*.([0-9]{14,14})"
    exit 1
}
...
MASCLR::log_message "Loaded the config file data" 3
initDB
...
while {[set record [gets $inFile]] != {} } {
    ...
    switch -exact [string range $record 42 44] {
        ...
        default { if { [string range $record 0 4] == "DFHDR"} {
...
MASCLR::log_message "Parsing $infile_name is done" 3
##
...
foreach {Iso bins up_path archive_path gen_filenm} $capIds {
    ...
    puts "short_name: $shortname"
    puts "file_name: $filename"
    ...
    if {$mode == "PROD"} {
		MASCLR::mutt_send_mail "reports-clearing@jetpay.com"  "$shortname" "" "$filename"
    } else {
         MASCLR::log_message "Report to be mailed : $filename" 3
		 MASCLR::mutt_send_mail $report_mail_id  "$shortname" "" "$filename"
    }

    if {[string first 121 $shortname] == 0} {
        exec cp $filename /clearing/filemgr/JOURNALS/INST_121/UPLOAD/
    }
    exec mv $filename $apath        
}
