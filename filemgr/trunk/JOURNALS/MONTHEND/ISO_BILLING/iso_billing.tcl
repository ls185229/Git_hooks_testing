...
set report_data_lst {
                    ...
                    {MONTHLY_SETTLE_RATE "Monthly Settle Rate" n}
...
proc do_report {} {
   ...
   default {
    ...
    formatted_puts "Fethching Monthly Settle Rate"

    set monthly_settle_qry "
        SELECT fp.institution_id, efp.entity_id, mf.rate_percent as MONTHLY_SETTLE_RATE
        FROM masclr.mas_fees mf
        INNER JOIN (SELECT DISTINCT institution_id, fee_pkg_id
            FROM masclr.fee_pkg)
            fp ON fp.fee_pkg_id = mf.fee_pkg_id AND fp.institution_id = mf.institution_id
            INNER JOIN (SELECT DISTINCT entity_id, institution_id, fee_pkg_id
                FROM masclr.entity_fee_pkg
                WHERE entity_id IN
                    (SELECT DISTINCT mtv.entity_id
                    FROM masclr.mas_trans_view mtv
                    WHERE mtv.gl_date >= '01-$date_my' and mtv.gl_date < '01-$next_mnth_my'
                        AND mtv.MAJOR_CAT = 'FEES'
                        AND mtv.MINOR_CAT != 'ACH'
        ...
        ON fp.fee_pkg_id = efp.fee_pkg_id
        AND fp.institution_id = efp.institution_id
        WHERE mf.mas_code = 'MONTHLY_SETTLE'
            AND mf.fee_status = 'C'"

    MASCLR::log_message "monthly_settle_qry : */\n$monthly_settle_qry;\n/*" 3

    orasql $cursor_C $monthly_settle_qry
    ...
    while {[orafetch $cursor_C -dataarray row -indexbyname] != 1403} {
        ...
        if {[dict exists $entity_dictionary $unique]} {
            dict set entity_dictionary $unique MONTHLY_SETTLE_RATE $row(MONTHLY_SETTLE_RATE)
        }
    }
...
if {![info exists email_arg]} {
    set email_lst "Reports-clearing.JetPayTX@ncr.com, accounting.JetPayTX@ncr.com"
} else {
...
#    append attachement_lst " -a [file tail $fl]"
     append attachement_list " [file tail $fl]"
  }

  set archivefile "IsoBilling.$filedate.$inst.zip"

  set compress_cmd "zip $archivefile $attachement_list"
  eval exec $compress_cmd

        # Email out all the reports
  # **********     COMMENT FOR TESTING      **********        
   MASCLR::mutt_send_mail "$email_lst"  "$email_subject" $email_body $archivefile
  

  
        # Clean up all the reports from this directory
	file copy -force $archivefile ./ARCHIVE/$archivefile
