...
################################################################################
# $Id: iso_billing.tcl 2731 2014-11-05 20:45:11Z mitra $
# $Rev: 2731 $
################################################################################
#
...
proc readCfgFile {} {
   ...
   global Shortname_list argv inst sql_inst card_type institution_arg
...
proc arg_parse {} {
    global argv date_arg MODE debug

    #scan for Date
    if { [regexp -- {(-[dD])[ ]+([0-9]{8,8})} $argv dummy1 dummy2 arg_date] } {
        formatted_puts "Date argument: $arg_date"
        set date_arg $arg_date
    }

    #scan for Test mode
    if { [regexp -- {(-[T])} $argv dummy1 dummy2 arg_test] } {
        formatted_puts "Test mode: on"
        set MODE "TEST"
    }         
...
proc do_report {} {
   ...
   default {
    ...
    set entity_qry  "
        select Entity_id, Entity_dba_Name, Entity_Name, institution_id, billing_type, default_mcc
    ...
    set sales_qry "
        select entity_id, institution_id, 
    ...
    while {[orafetch $cursor_C -dataarray row -indexbyname] != 1403} {
        ...
        if {[dict exists $entity_dictionary $unique]} {
            ...
            dict set entity_dictionary $unique NET_SALES_AMT    [expr $row(SALES_C_AMT) - $row(SALES_D_AMT)]
            dict set entity_dictionary $unique GROSS_SALES_AMT  $row(SALES_C_AMT)
            ...
            dict set entity_dictionary $unique SALES_REFUNDS    $row(SALES_REFUNDS)
        }
    }
    
    formatted_puts "Fetching card present/card not present"

    set IS_SALES "m.tid in ('010003005102', '010003005101', '010003005201', '010003005202', 
                            '010003005104', '010003005204', '010003005103', '010003005203')"

    set presentNotpresent_qry "
        SELECT m.institution_id, m.entity_id, 
               SUM(CASE WHEN (m.tid_settl_method = 'C' and i.pos_crd_present = '1') 
                        Then m.amt_original 
                        WHEN (m.tid_settl_method = 'D' and i.pos_crd_present = '1') 
                        Then -1*m.amt_original else 0 end) AS CP_AMOUNT, 
               SUM(CASE WHEN (m.tid_settl_method = 'C' and (i.pos_crd_present <> '1' or i.pos_crd_present is null )) 
                        Then m.amt_original 
                        WHEN (m.tid_settl_method = 'D' and (i.pos_crd_present <> '1' or i.pos_crd_present is null )) 
                        Then -1*m.amt_original else 0 end) AS CNP_AMOUNT, 
               SUM(CASE WHEN (i.pos_crd_present = '1') 
                        Then 1 else 0 end ) AS CP_COUNT, 
               SUM(CASE WHEN (i.pos_crd_present <> '1' or i.pos_crd_present is null ) 
                        Then 1 else 0 end ) AS CNP_COUNT, 
    ...
    set IS_CASHBACK "tid in ('010003005103', '010003005105', '010003005121',
                             '010003005203', '010003005205', '010003005221')"

    set cash_adv_cnt_qry "
        SELECT institution_id, entity_id,
               SUM(case when ($IS_CASHBACK) then 1 else 0 end) as CASH_ADVANCE_CNT
        FROM masclr.mas_trans_log
    ...
    set disc_sales_qry "
        SELECT m.entity_id, m.institution_id, 
              ( SUM( CASE WHEN m.tid_settl_method = 'C' and $IS_SALES 
                          then m.AMT_ORIGINAL else 0 end ) 
              - SUM( CASE WHEN m.tid_settl_method = 'D' and $IS_SALES 
        ...
        FROM mas_trans_log m join acq_entity ae 
             on (m.entity_id = ae.entity_id and m.institution_id = ae.institution_id), merchant_flag mf 
    ...
    for {set i 0} {$i <= $mid_lst_indx} {incr i} {
        ...
        MASCLR::log_message "dialup_trnx_counts_qry : $dialup_trnx_counts_qry" 3
