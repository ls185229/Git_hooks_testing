...
#  exec echo "$argv0 failed to run" | mailx -r $mailfromlist -s "merric-headers.tcl" $mailtolist
  exit
}
...
puts "today: $today...$tomorrow: $tomorrow"
      ...
      set cur_file_name "./106.REPORT.HEADER.$CUR_JULIAN_DATEX.$name_date.001.csv"
      set file_name     "106.REPORT.HEADER.$CUR_JULIAN_DATEX.$name_date.001.csv"
      set cur_file      [open "$cur_file_name" w]
      puts $cur_file "ISO,106"
      puts $cur_file " "
...
#*** discover id ***
      orasql $header_cursor11 "select unique visa_id, discover_id
                                from merchant
                               where substr(visa_id,1,6) in ( '440339', '451084')"
...
#syang 03-31-2009 replace tid in ('010003005102',  '010003005101') with tid in ('010003005102', '010003005101', '010003005201', '010003005202')
  ...
  set tid_list2     "'010005050000', '010007050000'"
    ...
    set query_string_all " SELECT  unique decode(institution_id, '106', '106', 'ROLL') as INSTID, entity_id,
...
puts "query_string_all: $query_string_all  "                    
    ...
    while {[set syang [orafetch $header_cursor5 -dataarray all -indexbyname]] == 0} {
          ...
          if {$all(INSTID) == "106" &&  [string length $all(ENTITY_ID)] == "15"} {
          ...
          } elseif {$all(INSTID) == "106" &&  [string length $all(ENTITY_ID)] < "2"} {
...
#*** Terminal Info ***
      set query_terminal "select  unique m.visa_id as VID, count(t.tid) as TERM_CNT
                           ...
                           where t.mid = m.mid and
                                 substr(m.visa_id, 1, 6) in ('440339', '461084') and 
                                 t.active = 'A'
                           group by m.visa_id
                           order by m.visa_id"
      puts $query_terminal
...
exec echo "" | mutt -a $cur_file_name -s "$file_name" "reports-clearing@jetpay.com,accounting@jetpay.com"
