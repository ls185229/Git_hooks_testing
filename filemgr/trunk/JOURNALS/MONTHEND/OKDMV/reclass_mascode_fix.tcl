...
set log_filename "/clearing/filemgr/RETURNS/LOG/move.log.[clock format [ clock scan "-0 day" ] -format %Y%m]"
  ...
  if {[orafetch $mas_sql1 -dataarray mas_query1 -indexbyname] != 1403} {
    ...
    if {[orafetch $mas_sql1 -dataarray mas_query1 -indexbyname] != 1403 } {
      ...
      if {[orafetch $mas_sql1 -dataarray mas_query1 -indexbyname] != 1403 } {
        ...
        exec echo "No interchange found for $submitted for a reclass transaction" | mutt -s "Problem with fixing reclass transactions" -- clearing@jetpay.com
      }
    } else {
    ...
    exec echo "No interchange information found for $mas_query(TRANS_SEQ_NBR)" | mutt -s "Problem with fixing reclass transactions" -- clearing@jetpay.com
  }
}
