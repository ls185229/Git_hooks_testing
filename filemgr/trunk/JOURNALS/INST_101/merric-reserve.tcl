...
set query_string "select unique entity_id from mas_trans_log where institution_id = '101'"
puts $query_string
...
set cur_file_name "./UPLOAD/101.REPORT.RESERVE.$CUR_JULIAN_DATEX.$filedate.001.csv"
set file_name "101.REPORT.RESERVE.$CUR_JULIAN_DATEX.$filedate.001.csv"
...
while {[orafetch $test_cursor1 -dataarray t -indexbyname] != 1403} {
  ...
  set query_string "select rsrv_percentage,parent_entity_id,entity_dba_name,creation_date,rsrv_fund_status,entity_status from acq_entity where entity_id = '$t(ENTITY_ID)' and institution_id = '101'"
  ...
  if {$STATUS_CODE != "F"} {
    set query_string "select sum(amt_original) as total from mas_trans_log where tid like '01000705%' and ENTITY_ID = '$t(ENTITY_ID)' and institution_id = '101'"
    orasql $fetch_cursor $query_string
    while {[orafetch $fetch_cursor -dataarray s -indexbyname] != 1403} {
      set RESERVE $s(TOTAL)
    }
