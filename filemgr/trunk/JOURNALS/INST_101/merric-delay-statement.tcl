...
#set query_string "select unique entity_id from mas_trans_log where ACTIVITY_DATE_TIME = '$date' and (institution_id,settl_plan_id) in (select institution_id,SETTL_PLAN_ID from settl_plan_tid where delay_factor0 <> '0' and delay_factor1 <> '0' and institution_id = '101')  and institution_id = '101'"
set query_string "select unique entity_id from mas_trans_log where (institution_id,settl_plan_id) in (select institution_id,SETTL_PLAN_ID from settl_plan_tid where delay_factor0 <> '0' and delay_factor1 <> '0' and institution_id = '101')  and institution_id = '101'"
puts $query_string
...
set cur_file_name "./UPLOAD/101.REPORT.PAYABLES.$CUR_JULIAN_DATEX.$filedate.001.csv"
set file_name "101.REPORT.PAYABLES.$CUR_JULIAN_DATEX.$filedate.001.csv"
...
while {[orafetch $test_cursor1 -dataarray t -indexbyname] != 1403} {
  ...
  set query_string "select parent_entity_id,entity_dba_name,actual_start_date from acq_entity where entity_id = '$t(ENTITY_ID)' and institution_id = '101'"
...
set query_string "select unique tid from mas_trans_log where date_to_settle > '$date' and (institution_id,settl_plan_id,tid) in (select institution_id,SETTL_PLAN_ID,tid from settl_plan_tid where delay_factor0 <> '0' and delay_factor1 <> '0' and institution_id = '101')  and institution_id = '101' and entity_id = $ENTITY_ID and activity_date_time < '$tomorrow'"
  orasql $fetch_cursor $query_string
  while {[orafetch $fetch_cursor -dataarray w -indexbyname] != 1403} {

  set query_string "select * from mas_trans_log where date_to_settle >  '$date' and ENTITY_ID = '$t(ENTITY_ID)' and institution_id = '101' and tid like '$w(TID)' and activity_date_time < '$tomorrow' and institution_id = '101'"
  puts $query_string
  orasql $fetch_cursor $query_string
  while {[orafetch $fetch_cursor -dataarray s -indexbyname] != 1403} {
      puts $s(TID)
