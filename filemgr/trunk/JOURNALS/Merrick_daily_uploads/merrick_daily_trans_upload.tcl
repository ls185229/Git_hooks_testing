...
# $Id: merrick_daily_trans_upload.tcl 2304 2013-05-16 19:06:44Z mitra $
...
global programName mailfromlist mailtolist logFilePtr
...
proc openOutputFile {} {
    ...
    set fileName [format "merrick_daily_trans_upload_%s.csv" $transDate ]
...
proc writeLogRcd {proc_cd message} {
  ...
  set logFileName [format "LOGS/merrick_daily_trans_upload_%s.log" $logDate ]
...
from (select ae.institution_id
      ...
      where trunc(mtl.gl_date) = to_date('$transDate', 'yyyymmdd')  
...
while {[orafetch $transRcdCurHandle -dataarray ds -indexbyname] != 1403} {
   ...
   set legalName         "$ds(LEGAL_NAME)"
   set mcc               "$ds(MCC)"
   set merchantId        "$ds(MERCHANT_ID)"
   set merchantName      "$ds(MERCHANT_NAME)"
   set moto              "$ds(MOTO)"
