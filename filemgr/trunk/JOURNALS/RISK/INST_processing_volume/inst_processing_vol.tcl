...
################################################################################
# $Id: inst_processing_vol.tcl 2522 2014-01-23 19:25:16Z mitra $
# $Rev: $
################################################################################
#
# Monthly Institution Processing Volume Risk Report
#
# USAGE: inst_processing_vol.tcl -I nnn ?\[-D YYYYMMDD\] ?\[-sort column_name\]
#
################################################################################
...
proc arg_parse {} {
    global argv date_arg institutions_arg sort_columns
    ...
    if { $numInst > 0 } {
        ...
        foreach {fullmatch instID} $res_lst {
            lappend institutions_arg $instID
        }
    }
}
...
proc print_usage {} {
    ...
    puts "USAGE: $argv0 -I nnn ?\[-D YYYYMMDD\] ?\[-sort column_name\]"
...
proc do_report {inst} {
    ...
    set qry "SELECT mtl.institution_id, mtl.entity_id,mtl.CARD_SCHEME,
     ...
     where (mtl.entity_id = acq.entity_id and mtl.institution_id = acq.institution_id)
         ...
         and (mtl.PAYMENT_SEQ_NBR, mtl.institution_id) in (select PAYMENT_SEQ_NBR, institution_id
                                 from ACCT_ACCUM_DET
    ...
    foreach sort_argument $sort_columns {
        ...
        set filename "inst.$inst.processing.volume.[clock format [clock scan $sdate -format %d-%b-%Y] -format %Y%m].[string tolower $sort_argument].csv"
...
proc do_email {$institutions_arg} {

    global institutions_arg

    log_msg "Start of emailing Risk Reports"

    set mdate   [clock format [clock seconds] -format "%Y%m"]
    set email_recipient "risk@jetpay.com,Reports-clearing@jetpay.com"
    set email_body "\nAttached are the Monthly Risk Reports for institution $institutions_arg. "
    set filelist ""
    set attachement_lst ""
    set locpth ""
    set email_subject " inst $institutions_arg Monthly Risk Reports $mdate"
    set locpth "/clearing/filemgr/JOURNALS/RISK/INST_processing_volume"

    cd $locpth

        # Search for only files belonging to the given day
    set filelist [glob -nocomplain -directory "./" -types f "inst.$institutions_arg.*.$mdate*.csv"]
    ...
    set mailpipe [open "| mutt -s \"$email_subject\" $attachement_lst -- $email_recipient" w ]
   ...
   log_msg "End of emailing Risk Reports"
...
if {![info exists institutions_arg]} {
    ...
    puts "No institution ID provided"
    print_usage
    cleanup_log
    exit
}
...
log_msg "Institution: $institutions_arg"
