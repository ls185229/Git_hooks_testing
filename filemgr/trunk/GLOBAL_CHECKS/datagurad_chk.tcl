...
# $Id: datagurad_chk.tcl 3420 2015-09-11 20:05:17Z bjones $
...
# exec echo "export ORACLE_SID trnclr4"
# exec echo "export TWO_TASK trnclr4"
...
proc connect_to_db {} {
    global db_logon_handle db_connect_handle msubj_c msubj_u msubj_h msubj_m msubj_l mbody_c mbody_u mbody_h mbody_m mbody_l box clrpath maspath mailtolist mailfromlist clrdb authdb handle
...
if {[catch {set handle [oralogon masclr/masclr@trnclr4]} result] } {
        puts "Encountered error $result while trying to connect to DB"
        set mbody "ALL PROCESSES STOPED SINCE reset_task.tcl could not logon to DB : Msg sent from reset_task.tcl\n $result"
        exec echo "$mbody_c $sysinfo $mbody" | mailx -r $mailfromlist -s "$msubj_c" $mailtolist
        exit 1
...
set sql1 "select count(1) as CNT from in_draft_main"
puts $sql1
orasql $get $sql1

orafetch $get -dataarray cnttrnclr1 -indexbyname

set sql2 "select count(1) as CNT from in_draft_main"
puts $sql2
orasql $get1 $sql2
