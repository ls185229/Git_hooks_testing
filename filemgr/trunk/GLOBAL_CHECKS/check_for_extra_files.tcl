...
################################################################################
# $Id: check_for_extra_files.tcl 3526 2015-10-04 23:00:30Z bjones $
################################################################################
#
# File Name:  check_for_extra_files.tcl
#
...
#    lappend DIRECTORIES "$CLRPATH/in/ipm"
#    set SKIP_FILES([lindex $DIRECTORIES 0]) { tvslog }
#    set EMPTY_FILE_RULES([lindex $DIRECTORIES 0]) { MOVE }
#
#    lappend DIRECTORIES "$CLRPATH/in/base2"
#    set SKIP_FILES([lindex $DIRECTORIES 1]) {base2testbed base2template base2simulator}
#    set EMPTY_FILE_RULES([lindex $DIRECTORIES 1]) { MOVE }
#
#    lappend DIRECTORIES "$CLRPATH/in/tc57"
#    set SKIP_FILES([lindex $DIRECTORIES 2]) {tvslog}
#    set EMPTY_FILE_RULES([lindex $DIRECTORIES 2]) { MOVE }
#
#    lappend DIRECTORIES "$CLRPATH/in/discover"
#    set SKIP_FILES([lindex $DIRECTORIES 3]) { tvslog }
#    set EMPTY_FILE_RULES([lindex $DIRECTORIES 3]) { MOVE }
#
#    lappend DIRECTORIES "$CLRPATH/in/mas"
...
#    set SKIP_FILES([lindex $DIRECTORIES 5]) {output.txt tvslog }
#    set EMPTY_FILE_RULES([lindex $DIRECTORIES 5]) { MOVE }
#
#    lappend DIRECTORIES "$CLRPATH/out/base2"
#    set SKIP_FILES([lindex $DIRECTORIES 6]) { tvslog }
#    set EMPTY_FILE_RULES([lindex $DIRECTORIES 6]) { MOVE }
#
#    lappend DIRECTORIES "$CLRPATH/out/tc57"
#    set SKIP_FILES([lindex $DIRECTORIES 7]) { tvslog }
#    set EMPTY_FILE_RULES([lindex $DIRECTORIES 7]) { MOVE }
#
#    lappend DIRECTORIES "$CLRPATH/out/discover"
#    set SKIP_FILES([lindex $DIRECTORIES 8]) { tvslog }
#    set EMPTY_FILE_RULES([lindex $DIRECTORIES 8]) { MOVE }
#
#    lappend DIRECTORIES "$CLRPATH/out/mas"
#    set SKIP_FILES([lindex $DIRECTORIES 9]) { tvslog }
#    set EMPTY_FILE_RULES([lindex $DIRECTORIES 9]) { MOVE }


#    lappend DIRECTORIES "$MASPATH/in"
#    set SKIP_FILES([lindex $DIRECTORIES 10])
#    set EMPTY_FILE_RULES([lindex $DIRECTORIES 10])
#
#    lappend DIRECTORIES "$MASPATH/process"
#    set SKIP_FILES([lindex $DIRECTORIES 11]) {out tvslog}
#    set EMPTY_FILE_RULES([lindex $DIRECTORIES 11]) { IGNORE }
#
#    lappend DIRECTORIES "$MASPATH/reject"
#    set SKIP_FILES([lindex $DIRECTORIES 12]) {mas_counting_parser_v13.tcl}
#    set EMPTY_FILE_RULES([lindex $DIRECTORIES 12]) { IGNORE }

    set jul_date [clock format [clock seconds]  -format "%y%j"]
    set SPECIAL_GLOB([lindex $DIRECTORIES 12])  "*$jul_date*"

    set_one_dir "$MASPATH/in"       {tvslog mas_mon.txt mas_counting_parser_v13.tcl} { MOVE }
    set_one_dir "$MASPATH/process"  {tvslog out fr_dir.txt} { IGNORE }
    set_one_dir "$MASPATH/reject"   {mas_counting_parser_v13.tcl} { IGNORE }

    set_one_dir "$FILEMGRPATH/DEBIT/UPLOAD"  {} { IGNORE }
#    lappend DIRECTORIES "$FILEMGRPATH/DEBIT/UPLOAD"
#    set SKIP_FILES([lindex $DIRECTORIES 13]) {}
#    set EMPTY_FILE_RULES([lindex $DIRECTORIES 13]) { IGNORE }

#    set_one_dir "$MASPATH/process"  {tvslog out} { IGNORE }
#    lappend DIRECTORIES "$FILEMGRPATH/DEBIT/INST_101/UPLOAD"
#    set SKIP_FILES([lindex $DIRECTORIES 14]) {}
#    set EMPTY_FILE_RULES([lindex $DIRECTORIES 14]) { IGNORE }
#
#    set_one_dir "$MASPATH/process"  {tvslog out} { IGNORE }
#    lappend DIRECTORIES "$FILEMGRPATH/DEBIT/INST_105/UPLOAD"
#    set SKIP_FILES([lindex $DIRECTORIES 15]) {}
#    set EMPTY_FILE_RULES([lindex $DIRECTORIES 15]) { IGNORE }
#
#    set_one_dir "$MASPATH/process"  {tvslog out} { IGNORE }
#    lappend DIRECTORIES "$FILEMGRPATH/DEBIT/INST_107/UPLOAD"
#    set SKIP_FILES([lindex $DIRECTORIES 16]) {}
#    set EMPTY_FILE_RULES([lindex $DIRECTORIES 16]) { IGNORE }
#
#    set_one_dir "$MASPATH/process"  {tvslog out} { IGNORE }
#    lappend DIRECTORIES "$FILEMGRPATH/DEBIT/INST_121/UPLOAD"
#    set SKIP_FILES([lindex $DIRECTORIES 16]) {}
#    set EMPTY_FILE_RULES([lindex $DIRECTORIES 16]) { IGNORE }
