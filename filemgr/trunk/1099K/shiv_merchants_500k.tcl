...
#
################################################################################
# $Id: shiv_merchants_500k.tcl 4012 2017-01-06 18:13:25Z millig $
# $Rev: 4012 $
# $Author: millig $
################################################################################
    ...
    if {[catch {set clrHandle2 [oraopen $clrDBLoginHandle]} result ]} {
       ...
       set merchantTrans "select m.visa_id,
               ...
               , visa_id  as acctnumbermid
                        ,sum (case WHEN (th.card_type = 'AX' and request_type in ('0200', '0250'))  then amount else 0 end) AS AMPAYMENT
                    ...
                    FROM MERCHANT m 
                        ...
                        and ((substr(m.visa_id,1,11) = '45404527400') or (substr(m.visa_id,1,11) = '45404547400'))
                        and m.active = 'A'
                        and m.amex_se_no != ' '
                        and mm.shortname = 'JETPAYSQ'
                        and ((th.card_type = 'AX') OR (th.card_type = 'VS') or (th.card_type = 'MC') OR (th.card_type = 'DS'))
                        and amount > 0
                        group by m.visa_id,amex_se_no,long_name,phone_no,address_1,address_2,city,state,zip"
...
proc openOutputFile {} {
    ...
    set fileName [format "shiv_amex.csv"]
...
puts $filePtr "ACCTNUMBERMID,AMEX-SE-NUM,GROSSAMT,AMEXAMT,VISAAMT,MASTERCARDAMT,DISCOVERAMT,NAMELINE1,PHONE,ADDR1,ADDR2,CITY,USSTATE,USZIPCODE"
...
while {[orafetch $merchantSumCurHandle -dataarray ds3 -indexbyname] != 1403} {
         ...
         set amex                        "$ds3(AMPAYMENT)"
         set visa                        "$ds3(VSPAYMENT)"
         set master                      "$ds3(MCPAYMENT)"
         set discover                    "$ds3(DSPAYMENT)"
	     set  gross $amex   
		 set  gross [expr $gross + $visa      ]
		 set  gross [expr $gross + $master    ]
		 set  gross [expr $gross + $discover  ]

	if {(($amex > 0) && ($amex <= 500000))} {
	  set detailLine "\"$acctnumbermid\",\"$amex_se_no\",\"$gross\",\"$amex\",\"$visa\",\"$master\",\"$discover\",\"$nameline1\",\"$phone\",\"$addr1\",\"$addr2\",\"$city\",\"$usstate\",\"$uszipcode\","
