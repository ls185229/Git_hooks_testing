...
#
################################################################################
# $Id: create_121_1099K_file.tcl 4012 2017-01-06 18:13:25Z millig $
# $Rev: 4012 $
# $Author: millig $
################################################################################
    ...
    set merchantInfoCur "select distinct institution_id iso_id
          ...
          where contact_role = 'DEF'
            ...
            and (ae.entity_status = 'A'
               or ae.termination_date between to_date('01/01/2013', 'mm/dd/yyyy') and to_date('01/14/2014', 'mm/dd/yyyy'))
          	and ae.entity_level = 70 
            and ae.institution_id = '121'"
	...
	set merchantTrans "select posting_entity_id
                    ...
                    from (select posting_entity_id
                         ...
                         , decode(to_char(gl_date, 'mm'), '12', 1, 0)  DECCOUNT
                                        from mas_trans_log
                                        where institution_id in ('105', '121') 
...
proc openOutputFile {} {
    ...
    set fileName "INST-121_1099K_20140122.csv"
...
while {[orafetch $merchantInfoCurHandle -dataarray ds -indexbyname] != 1403} {
   ...
   set countTotal 0
 	  orabind $clrHandle2 :ENTITY $acctnumbermid :YEAR $year
