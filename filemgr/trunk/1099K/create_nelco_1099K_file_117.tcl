...
################################################################################
#
#    File Name - create_visa_ammf
#
#    Description - Read the MASCLR db to build the AMMF records to be sent to Visa
#
...
#
################################################################################
# $Id: create_nelco_1099K_file_117.tcl 4012 2017-01-06 18:13:25Z millig $
# $Rev: 4012 $
# $Author: millig $
################################################################################
...
set merchantInfoCur "select distinct trim(to_char(tax_id, '000000009')) tin
          ...
          where contact_role = 'DEF'
            ...
            and ae.institution_id in ($tempInstId)
...
puts "T-1 merchantInfoCur $merchantInfoCur"
...
set instId "106"
    ...
    set sqlStmt "select trim(to_char(nvl(sum(amt_billing), 0), '99999999.99')) GROSSPAYMENTAMT
                 ...
                 where institution_id = '$instId'
                   ...
                   and gl_date between to_date('01/01/$year', 'mm/dd/yyyy') and to_date('03/31/$year', 'mm/dd/yyyy')"
	   
    set merchantSumCurHandle [executeQuery $sqlStmt $clrHandle2 ]

puts "T-1 sqlStmt -$sqlStmt"
