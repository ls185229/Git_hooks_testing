...
set new_file_name "./ARCHIVE/101.QUALIFICATION-$fdate.tex"
set pdf_file_name "./101.QUALIFICATION-$fdate.pdf"
...
where mas_trans_log.entity_acct_id = entity_acct.entity_acct_id AND
      ...
      mas_trans_log.institution_id = '101' AND
      acq_entity.institution_id = '101' AND
      entity_acct.institution_id = '101'
group by entity_acct.owner_entity_id,
         ...
         tid.tid_settl_method,
         mas_trans_log.trans_ref_data
order by acq_entity.entity_name,
         mas_trans_log.activity_date_time,
         card_scheme.card_scheme_name,
         mas_code.mas_desc,
         mas_trans_log.fee_pkg_id,
...
#  puts "total:$total + Trans Total:$t(AMOUNT) $t(MAS_DESC) $t(ENTITY_ID)"
  ...
  set sql_string "select to_char(sum(AMT_ORIGINAL), '999999.99') as fee from mas_trans_log where mas_code = '$t(MAS_CODE)' and trans_ref_data = '$t(TRANS_REF_DATA)' and FEE_PKG_ID != 0 and trans_seq_nbr = '$t(TRANS_SEQ_NBR)' and institution_id = '101'"
 # set sql_string "select to_char(sum(AMT_ORIGINAL), '999999.99') as fee from mas_trans_log where mas_code = '$t(MAS_CODE)' and trans_ref_data = '$t(TRANS_REF_DATA)' and FEE_PKG_ID != 0"
  ...
  if {$s(FEE) == ""} {
        set s(FEE) 0
	puts "$t(TRANS_REF_DATA) :: $s(FEE) :: $t(ENTITY_NAME)"
  }
...
puts $new_file "\\cline{4-6}"
if {$entity_id == "YES"} {
  puts $new_file "$t(ENTITY_NAME)& Qualification Sub Total & & [format "%0.2f" $qual_total] & [format "%0.2f" $qual_fee_total]& [format "%0.2f" [expr $qual_total - $qual_fee_total]]\\\\"
...
#exec cat body.txt dummy.uuencode | mailx -r clearing@jetpay.com -s "QUALIFICATION Detailed Reports for $fdate" clearing@jetpay.com 
#josh@jetpay.com veronica@jetpay.com
exec mv ./$pdf_file_name ./ARCHIVE/$pdf_file_name
exec rm 101.QUALIFICATION-$fdate.aux 101.QUALIFICATION-$fdate.log
