...
#New code for qualification summary for institution 107
#Code version # 7.3
...
set new_file_name "./ARCHIVE/107.QUALIFICATION-SUMM_$fdate.tex"
set pdf_file_name "107.QUALIFICATION-SUMM_$fdate.pdf"
...
#} 
#
# SYANG: according to Rifat,  107 does not refund on fees any more. Refund on sales needs to be put in count. 05-05-08

            if {[string range $t(TID) 0 7] == "01000402"} {
                  if { $t(TID) == "010004020000" } {
                    set trn_net_amt [expr $t(AMT) - $t(INT)]
                  } elseif { $t(TID) == "010004020005" } {
                    set t(AMT) 0
                  }
            } elseif { $t(TID) == "010003005102"} {
                  set t(AMT) [expr $t(AMT) * -1]
                  set t(INT) 0
                  set trn_net_amt [expr $t(AMT) - $t(INT)]
            } else {
...
#exec scp $pdf_file_name filemgr@dfw-adm-web-07:./IQR
...
exec cat body.txt dummy.uuencode | mailx -r clearing@jetpay.com -s "107-QUALIFICATION Summary Reports for $fdate" clearing@jetpay.com
#josh@jetpay.com veronica@jetpay.com
exec mv ./$pdf_file_name ./ARCHIVE/$pdf_file_name
exec rm 107.QUALIFICATION-SUMM_$fdate.aux 107.QUALIFICATION-SUMM_$fdate.log
