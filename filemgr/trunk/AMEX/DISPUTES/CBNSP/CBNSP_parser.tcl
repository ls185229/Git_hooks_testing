...
58  SELLER_ID                     20 Character_spaces      1005   1024  varchar
59  FILLER                         5 Character_spaces      1025   1029  skip
...
set rcdCnt 0
   ...
   while { [gets $filePtr line] >= 0 } {
    ...
    if {$rcdCnt == 0} {
       puts "No Amex Chargebacks today in $inputFile"
       exit 0
    } else {
       puts "$rcdCnt Amex Chargebacks today in $inputFile"
    }
}
...
set insertValue ""
set insertColumn ""
set cbTid "010103015101"
set cbRvrslTid "010103015201"
set msgType "0000"
set cardScheme "03"
set errFlag 0
    ...
    while { [gets $filePtr line] >= 0 } {
        ...
        ### Data Edits on detail record
          if { [ string range $line 0 0 ] == "D" } {
           ...
           ### Default currency is USD (code 840) with two digits after the decimal
           ...
           foreach {num name length type start end db_type} $detail_record {
			  ...
			  } elseif {$name == "TRIUMPH_SEQ_NO" } {
				if {$DetailValue(TRIUMPH_SEQ_NO) > 0 } {
                     append insertColumn "$name,"
                     append insertValue "\'$DetailValue($name)\',"
                 } else {
                     append insertColumn "$name,"
                     append insertValue "\'00\',"
                 }                 
               } else {
                  append insertColumn "$name,"
                  append insertValue "\'$DetailValue($name)\',"
		...
		if {$DetailValue(FIN_IND) == "Y"} {
           ...
           if {$DetailValue(CB_AMOUNT) < 0} {
              set tid $cbTid
              set amount [expr {$DetailValue(CB_AMOUNT) * -100} ]
              set amount  [lindex [split $amount .] 0]
              puts "T-1 amount after multiplying $amount"
           ...
           set query "insert into in_draft_main
                      (TRANS_SEQ_NBR, 
                       ...
                       ('$transSeqNbr', 
						...
						'$currency_code',
						'2',
						to_date\(\'$DetailValue(DATE_OF_CHARGE)\', \'YYYYMMDDHH24MISS\'\),
                        '$DetailValue(IND_REF_NUMBER)',
						'0',
                        '$entity_id',
						'0',
						'$DetailValue(NOTE5)',
