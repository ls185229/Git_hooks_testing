...
#                                                                                #
#   Author:      Debra Evans                                                     #
#   File Name:   get_amex_tilr.sh                                                #
#   Description: Startup script for get_amex_tilr.exp                            #
#                This script does the following:                                 #
#                1. Invoke the process get_amex_tilr.ex p                        #
#                2. Get AMEX TILR(EPTRN) file                                    #
...
log_file=/clearing/filemgr/AMEX/LOG/amex_dnload_tilr.log
...
$locpth/get_amex_tilr.exp $log_file >> $log_file
rc=$?
if [ $rc != 0 ]
then
   print "return code is $rc"
   m_reason="get_amex_tilr.exp Download of AMEX TILR file failed with $rc at `date +%Y%m%d%H%M%S` -- check log $log_file \n"
   subject=$msubj_c
   mail_someone "$m_reason"
   print "Ending get_amex_tilr.exp Download of AMEX TILR file failed at `date +%Y%m%d%H%M%S` \n" | tee -a $log_file
fi
