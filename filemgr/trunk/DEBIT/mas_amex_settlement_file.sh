...
################################################################################
# $Id: mas_amex_settlement_file.sh 3512 2015-09-22 21:33:12Z bjones $
# $Rev: 3512 $
################################################################################
#
# File Name:  mas_amex_settlement_file.sh
#
...
inst_id="107"
sname="OKDMV"
logfile="amex_mas_file.log"

print "Beginning at `date +%Y%m%d%H%M%S`" >> $locpth/LOG/$logfile

if ./mas_amex_settlement_file.tcl $sname $inst_id $1 >> $locpth/LOG/$logfile; then
        print "Script mas_amex_settlement_file.tcl $sname $inst_id $1 completed successfully" | tee -a $locpth/LOG/$logfile
else
        mbody="$SYS_BOX Script mas_amex_settlement_file.tcl $sname $inst_id $1 Failed"
