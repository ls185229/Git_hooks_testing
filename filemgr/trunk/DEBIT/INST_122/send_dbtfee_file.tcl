...
################################################################################
#
# File Name:  send_dbfee_file.tcl
#
# Description:  This script send the institution PIN debit fee file to MAS.
#              
...
if {[file exists ./UPLOAD/debit.skip]} {
 set mbody "No Debit fee file generated skipping file upload to MAS"
### exec echo "$mbody_l $sysinfo $mbody" | mailx -r $mailfromlist -s "$msubj_l" clearing@jetpay.com
 file delete ./UPLOAD/debit.skip
...
catch {set y [exec find $env(PWD)/UPLOAD -mtime 0 | grep $inst_id.DEBITFEE.01.*]} result 

#if {$y == ""} {
#exec echo "NO FILE FOUND" | mailx -r clearing@jetpay.com -s "PROD: MAS: DEBITRAN file not found in MAS folder" clearing@jetpay.com assist@jetpay.com
#catch {file delete /clearing/filemgr/process.stop} result
#exit 1
#}
#set i 0
#if {[llength $y] >=  2} {
#exec echo $x | mailx -r clearing@jetpay.com -s "PROD: MAS: Multiple DEBITRAN file found in MAS folder" clearing@jetpay.com assist@jetpay.com
#exit 1
#}

if {[llength $y] ==  1} {
puts $y
set counting [file tail $y]
puts $counting
exec chmod 775 $env(PWD)/UPLOAD/$counting 
}

set dummy ""
set dummy [exec cp -p $env(PWD)/UPLOAD/$counting $maspath/data/in]
puts $dummy
#if {$dummy != ""} {
