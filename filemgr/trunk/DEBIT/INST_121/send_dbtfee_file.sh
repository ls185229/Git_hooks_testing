...
#
# Description:  This script send the institution PIN debit file to MAS.
#              
...
if send_dbtfee_file.tcl >> $locpth/LOG/xfercnt.log; then
	...
	echo "Script send_dbtfee_file.tcl for $inst_id was not run for today" | mailx -r $MAIL_FROM -s "$SYS_BOX: MAS: $locpth/send_dbtfee_file.tcl was not run" $ALERT_NOTIFICATION
fi
