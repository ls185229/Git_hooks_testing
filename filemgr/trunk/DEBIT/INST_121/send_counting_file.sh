...
################################################################################
#
# File Name:  send_counting_file.sh
#
# Description:  This script intitiates movement of the institution counting file 
#               to MAS.
#              
...
print "Beginning  COUNTING FILE xfer for JETPAYESQ at `date +%Y%m%d%H%M%S`" | tee -a $locpth/LOG/xfercnt.log
...
if send_counting_file.tcl >> $locpth/LOG/xfercnt.log; then
	...
	echo "Script send_counting_file.tcl for $inst_id was not run for today" | mailx -r $MAIL_FROM -s "$SYS_BOX: MAS: $locpth/send_counting_file.tcl was not run" $ALERT_NOTIFICATION
fi

print "Ending COUNTING FILE xfer for JETPAYESQ at `date +%Y%m%d%H%M%S`" | tee -a $locpth/LOG/xfercnt.log
