...
################################################################################
# $Id: mas_debit_fee_file.sh 3512 2015-09-22 21:33:12Z bjones $
# $Rev: 3512 $
################################################################################
#
# File Name:  mas_debit_fee_file.sh
#
# Description:  This script initiates the creation of a MAS fee file for PIN
#               debit activity.
#              
...
print "Beginning at `date +%Y%m%d%H%M%S`" >> $locpth/LOG/$logfile
    ...
    if ./mas_debit_fee_file.tcl $sname $inst_id >> $locpth/LOG/$logfile; then
	        print "Script mas_debit_fee_file.tcl $sname $inst_id completed successfully" | tee -a $locpth/LOG/$logfile
    else
	        mbody="$SYS_BOX Script mas_debit_fee_file.tcl $sname $inst_id Failed"
