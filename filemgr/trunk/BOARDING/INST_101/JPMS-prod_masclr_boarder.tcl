...
orasql $get $updt_sq
  ...
  set m_type [string range [string toupper [lindex $cur_line 0]] 0 0]
  set dba [string toupper [lindex $cur_line 1]]
  set ent_id [string trim [string toupper [lindex $cur_line 2]]]
  set zero 0
...
set pln_desc $dba_nm
set ent_nm $dba_nm
  set mcc [lindex $cur_line 3]
		set mcc "[string map {- "" + "" & "" _ "" ' "" # "" @ "" / "" ? "" ! "" % "" $ "" * "" ( "" ) "" , "" \  ""} $mcc]"
  set bin [lindex $cur_line 4]
		set bin "[string map {- "" + "" & "" _ "" ' "" # "" @ "" / "" ? "" ! "" % "" $ "" * "" ( "" ) "" , "" \  ""} $bin]"
  set tax_id [lindex $cur_line 5]
		set tax_id "[string map {- "" + "" & "" _ "" ' "" # "" @ "" / "" ? "" ! "" % "" $ "" * "" ( "" ) "" , "" \  ""} $tax_id]"
  set address [string toupper [lindex $cur_line 6]]
...
puts $ph1
  ...
  set fax [lindex $cur_line 25]
		set fax "[string map {- "" + "" & "" _ "" ' "" # "" @ "" / "" ? "" ! "" % "" $ "" * "" ( "" ) "" , "" \  ""} $fax]"
  if {$fax == 0} {
    set fax "" 
  }

  set bnk_nm [lindex $cur_line 1]
		set bnk_nm "[string toupper [string map {- \  + "" & "" _ "" ' "" # "" @ "" / "" ? "" ! "" % "" $ "" * "" ( "" ) "" , ""} $bnk_nm]]"
  if {$bnk_nm == 0} {
    set bnk_nm ""
  }
  set acno [lindex $cur_line 14]
		set acno "[string map {- "" + "" & "" _ "" ' "" # "" @ "" / "" ? "" ! "" % "" $ "" * "" ( "" ) "" , "" \  ""} $acno]"
  if {$acno == 0} {
    set acno ""
  }

  set trn [lindex $cur_line 13]
		set trn "[string map {- "" + "" & "" _ "" ' "" # "" @ "" / "" ? "" ! "" % "" $ "" * "" ( "" ) "" , "" \  ""} $trn]"
  if {$trn == 0} {
    set trn ""
  }
  set prdt_type [lindex $cur_line 0]
  if {$trn == 0} {
   puts $cur_line
  ...
  set curncy_cd [string trim [lindex $cur_line 11]]
  ...
  set cntry_cd [string trim [lindex $cur_line 12]]
  ...
  set process_type [string toupper [string range [string trim [lindex $cur_line 0]] 0 0]]
...
#set strn "124384602"
#set sacno "5001"
   set get_sacct "select * from settl_acct where institution_id = '$inst_id'"
	orasql $cursor $get_sacct
	orafetch $cursor -dataarray s_acno -indexbyname
	if {[oramsg $info_get rc] != 1403 } {
		set strn $s_acno(SETTL_ROUTING_NBR)
		set sacno $s_acno(SETTL_ACCT_NBR)
	} else {
		puts "Could not find settle account number for $inst_id"
		exit 1
	} 
