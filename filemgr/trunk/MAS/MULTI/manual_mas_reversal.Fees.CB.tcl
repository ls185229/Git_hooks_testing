...
set get_ent "select * from in_draft_main where trans_seq_nbr in (
...
###### Writing Massage Detail records
   ...
   #T-1 this logic if for CB if mas code is null in IDM


   if {$a(MAS_CODE) == ""} {
      if {$a(TID) == "010103015101"} {
         if {$a(CARD_SCHEME) == "04"} {
            set rec(mas_code) "04CHGBCK"
         } elseif {$a(CARD_SCHEME) == "05"} {
            set rec(mas_code) "05CHGBCK"
         } else {
            set rec(mas_code) $a(MAS_CODE)
         }
      }
