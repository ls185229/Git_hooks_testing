...
################################################################################
#
# File Name:  mas_ach_txn_count_sum_file_v13.sh
#
...
print "Beginning  COUNTING FILE at `date +%Y%m%d%H%M%S`" | tee -a $locpth/LOG/achcnt.log

if mas_ach_txn_count_sum_file_v13.tcl $sht_name $inst_id `date +%Y%m%d` >> $locpth/LOG/achcnt.log; then
	print "Script mas_ach_txn_count_sum_file_v13.tcl completed." | tee -a $locpth/LOG/achcnt.log
else
	echo "Script mas_ach_txn_count_sum_file_v13.tcl for $inst_id failed" | mailx -r $MAIL_FROM -s "$SYS_BOX: MAS: $locpth/mas_ach_txn_count_sum_file_v13.tcl Failed" $MAIL_TO
fi

print "Ending COUNTING FILE xfer for JETPAY at `date +%Y%m%d%H%M%S`" | tee -a $locpth/LOG/achcnt.log
fi
