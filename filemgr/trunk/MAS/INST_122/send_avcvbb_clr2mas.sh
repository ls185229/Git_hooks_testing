...
################################################################################
#
# File Name:  send_avcvbb_clr2mas.sh
#
# Description:  This program initiations sending of a MAS AVS/CVV and BIN
#               blocking transaction count summary file to MAS for the 
#               institution.
#              
...
if $locpth/$codename $sht_name $inst_id `date +%Y%m%d` >> $locpth/LOG/$lgfname; then
	print "Script $codename completed" | tee -a $locpth/LOG/$lgfname
else
	echo "Script $locpth/$codename $sht_name $inst_id `date +%Y%m%d` failed" | mailx -r $mailfromlist -s "$box :: MAS :: Priority Critical- Call On-Call Engr. immidiately" $mailtolist 
fi
