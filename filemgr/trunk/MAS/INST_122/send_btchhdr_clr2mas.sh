...
################################################################################
#
# File Name:  send_btchhdr_clr2mas.sh
#
# Description:  This program initiations sending of a MAS batch header file to 
...
print "Beginning  CLEARING FILE TRANSFER at `date +%Y%m%d%H%M%S`" | tee -a $locpth/LOG/btchhdr.log

if send_btchhdr_clr2mas.tcl $inst_id >> $locpth/LOG/btchhdr.log; then
	print "Script send_btchhdr_clr2mas.tcl completed" | tee -a $locpth/LOG/btchhdr.log
else
	echo "Script $locpth/send_btchhdr_clr2mas.tcl for $inst_id failed" | mailx -r $MAIL_FROM -s "$SYS_BOX : MAS: send_btchhdr_clr2mas.tcl Failed" $MAIL_TO
fi

print "Ending CLEARING FILE TRANSFER at `date +%Y%m%d%H%M%S`" | tee -a $locpth/LOG/btchhdr.log
