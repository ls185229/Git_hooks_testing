...
#
# Description:  This program initiations sending of a MAS counting file to 
...
if send_counting_file.tcl >> $locpth/LOG/xfercnt.log; then
	...
	echo "Script send_counting_file.tcl for $inst_id failed" | mailx -r $MAIL_FROM -s "$SYS_BOX: MAS: $locpth/send_counting_file.tcl Failed" $MAIL_TO
fi
