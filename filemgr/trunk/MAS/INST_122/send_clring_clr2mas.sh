...
################################################################################
#
# File Name:  send_clring_clr2mas.sh
#
# Description:  This program initiations sending of a MAS clearing file to 
...
if send_clring_clr2mas.tcl $inst_id >> $locpth/LOG/xferclr.log; then
	print "Script send_clring_clr2mas.tcl $inst_id completed" | tee -a $locpth/LOG/xferclr.log
else
	echo "Script $locpth/send_clring_clr2mas.tcl $inst_id failed" | mailx -r $MAIL_FROM -s "$SYS_BOX : MAS: send_clring_clr2mas.tcl Failed" $MAIL_TO
	exit 1
fi
