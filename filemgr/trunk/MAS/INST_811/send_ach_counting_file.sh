...
if send_ach_counting_file.tcl >> $locpth/LOG/xfercnt.log; then
	print "Script send_ach_counting_file.tcl completed." | tee -a $locpth/LOG/xfercnt.log
else
	echo "Script send_ach_counting_file.tcl for $inst_id failed" | mailx -r $MAIL_FROM -s "$SYS_BOX: MAS: $locpth/send_ach_counting_file.tcl Failed" $MAIL_TO
fi
