...
#set rightnow "[string toupper [clock format [ clock scan "$currdate 1 day" ] -format "%Y%m%d"]]000000"
...
set get_ent "select * from ep_event_log where ems_item_type in ('CB1','CB2','RR1','RR2') and event_log_date like '$e_date%' and event_status in ('NOTIFY', 'NEW')"
puts $get_ent
...
puts ">$b(CARD_SCHEME)<"
    ...
    switch $a(EMS_ITEM_TYPE) {
        "CB1"   {
                         ...
                         } elseif {$c(TID) == "010103015202"} {
                            if {$b(CARD_SCHEME) == "04"} {
                                set ms_cd "04CHGBCK"
                                # set mas_tid "010003015202"
...
set result1 ""
catch {exec cp -p $outfile $maspath/data/in/$outfile} result1
}
if {$result1 == ""} {
