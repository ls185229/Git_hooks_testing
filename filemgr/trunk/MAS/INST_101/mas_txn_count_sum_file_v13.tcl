...
set get_info "select mid, count(amount) as cnt, sum(amount) as amt, card_type, request_type, decode (action_code,'000','A','D') as status from $tblnm_tn where mid in ($midlist) and authdatetime >= $begintime || '$begint' and authdatetime < $endtime || '$endt' and shipdatetime < $endtime || '$endt' and request_type < '0600' group by mid, card_type, request_type, decode (action_code,'000','A','D') order by mid"
 ...
 if {$chkmid != $a(MID)} {
   if {$chkmid != " "} {
    ...
    puts "position 1: totamt = $totamt"
        set rec(batch_amt) [expr wide([expr $totamt * 100])]
...
if {[set mcdchk $mas_cd($c_sch-$a(REQUEST_TYPE)-$a(STATUS))] != "IGNORE"} {
        ...
        set rec(amt_original) [expr wide([expr $a(AMT) * 100])]
...
###### Writing File Trailer Record
        set rec(file_amt) [expr wide([expr $ftotamt * 100])]
