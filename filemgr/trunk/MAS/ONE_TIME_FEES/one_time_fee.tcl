...
################################################################################
# $Id: one_time_fee.tcl 4178 2017-05-09 21:26:44Z fcaron $
# $Rev: 4178 $
################################################################################
#
# File Name:  one_time_fee.tcl
#
...
proc gather_values_from_fee_file { database_handle fee_output_file_name verification_file_name } {
    ...
    set curr_code_cursor_query1 [MASCLR::open_cursor $database_handle]
...
=======
    set where_values ""
>>>>>>> f83a055cb17ec60b63eedfe8b3e956f63202d178
    #puts stdout "  feefile.txt should have this format: "
    #puts stdout "  entity ID,card_scheme,TID,mas code to use,next generate date,fee amount,fee description"
    ...
    while {$cur_line != ""} {
        ...
        set the_sql "select INSTITUTION_ID, ENTITY_ID, ENTITY_DBA_NAME from acq_entity where entity_id = '$ent_id'"
...
>>>>>>> f83a055cb17ec60b63eedfe8b3e956f63202d178
        ...
        set the_sql "SELECT inst_curr_code FROM INSTITUTION WHERE INSTITUTION_ID='$inst_id'"

        if { [catch {set curr_code [MASCLR::fetch_single_value $curr_code_cursor_query1 $the_sql]} failure ] } {
            error "$failure"
        }
        ...
        MASCLR::print_to_file_name $fee_output_file_name "insert into non_activity_fees (institution_id, non_act_fee_pkg_id, charge_method,tid, mas_code,   fee_amt, fee_curr, nbr_installments, threshold, generate_freq, gen_date_list_id, last_generate_date, next_generate_date, day_of_month, start_day_month, end_day_month, card_scheme)
             values ('$inst_id','$non_act_fee_pkg_id_var', 'I','$tid_to_use', '$mas_code_to_use', '$fee_amount', '$curr_code','1','0', 'D','0', to_date(to_char(sysdate-1 , 'DD-MON-YYYY')||' 12:00:00 AM', 'DD-MON-YYYY HH:MI:SS AM'), to_date('$next_gen_date','DD-MON-YYYY HH:MI:SS AM'), '0', '0', '0', '$card_scheme_var');"

        MASCLR::print_to_file_name $fee_output_file_name "\n"

        MASCLR::print_to_file_name $fee_output_file_name "insert into ent_nonact_fee_pkg (institution_id, entity_id, non_act_fee_pkg_id, start_date,end_date, inc_child_ent_flag)
             values ('$inst_id', '$ent_id', '$non_act_fee_pkg_id_var', to_date(to_char(sysdate-1, 'DD-MON-YYYY')||' 12:00:00 AM', 'DD-MON-YYYY HH:MI:SS AM'),to_date('$fee_end_date','DD-MON-YYYY HH:MI:SS AM'), 'N');"

        MASCLR::print_to_file_name $fee_output_file_name "\n"

        set cur_line  [split [set orig_line [gets $input_file_id] ] ,]
		
		#Storing keys (inst) and associated values (sequences) into a dictionary
		
		if {[info exists where_dict]} {
                MASCLR::log_message "where dict $where_dict" 3
                if  {[dict exists $where_dict $inst_id]} {
                    set curr_list [dict get $where_dict $inst_id]
                    MASCLR::log_message "where curr_list $curr_list" 3
                    set curr_list [list [lindex $curr_list 0] $non_act_fee_pkg_id_var]
                    MASCLR::log_message "where curr_list $curr_list" 3
...
>>>>>>> f83a055cb17ec60b63eedfe8b3e956f63202d178
                    ...
                    set curr_list [list $non_act_fee_pkg_id_var $non_act_fee_pkg_id_var ]
                }
            } else {
                set curr_list [list $non_act_fee_pkg_id_var $non_act_fee_pkg_id_var ]
            }
            dict set where_dict $inst_id $curr_list
<<<<<<< HEAD
	...
	#Pulling the stored keys and values from dictionary and assigning to where_clause to create verification query
	
	if {[info exists where_dict]} {
		foreach {a b} $where_dict { 
			
			append where_clause "(enfp.institution_id = '$a' and enfp.non_act_fee_pkg_id between "
			append where_clause [lindex $b 0]
			append where_clause " and " 
			#append where_clause [expr {[lindex $b 1] + 2 }] 
			append where_clause [lindex $b end] 
			#Checking last value to close parenthesis
			if {$count != [llength [dict keys $where_dict]] && [llength [dict keys $where_dict]] > 1} { 
				append where_clause ")\n or \n"
				incr count 1
...
>>>>>>> f83a055cb17ec60b63eedfe8b3e956f63202d178
    ...
    MASCLR::print_to_file_name $verification_file_name "    CASE WHEN t.tid_settl_method like 'C'  THEN 'Cr'"
    ...
    MASCLR::print_to_file_name $verification_file_name ""
    MASCLR::print_to_file_name $verification_file_name "join tid t"
    MASCLR::print_to_file_name $verification_file_name "on t.tid = naf.tid"
    MASCLR::print_to_file_name $verification_file_name ""
    ...
    MASCLR::print_to_file_name $verification_file_name "and (enfp.entity_id = mtl.posting_entity_id or enfp.entity_id = mtl.entity_id)"
    MASCLR::print_to_file_name $verification_file_name "and mtl.gl_date > to_date('$next_gen_date','DD-MON-YYYY HH:MI:SS AM') - 2"
    MASCLR::print_to_file_name $verification_file_name "and mtl.gl_date < to_date('$next_gen_date','DD-MON-YYYY HH:MI:SS AM') + 2"
    MASCLR::print_to_file_name $verification_file_name ""
...
>>>>>>> f83a055cb17ec60b63eedfe8b3e956f63202d178
    ...
    send_file $files_to_send_list {Clearing.JetPayTX@ncr.com} {"One time fees"} "One time fees generated on $MASCLR::CURRENT_EF_DB_DATE"
...
#
#       foreach inst_id {101 105 107 121 129 130 134 811} {
...
#select MASCLR.NON_ACT_FEE_PKG_ID_107.NEXTVAL from dual
#Old SEQ select LAST_SEQ_NBR from seq_ctrl

    set the_sql "select MASCLR.NON_ACT_FEE_PKG_ID_$inst_id.NEXTVAL from dual"
...
proc main {} {
#seq_ctrl_update_file_name -- Removed arg
    global argv fee_output_file verification_query_file_name
    global env
