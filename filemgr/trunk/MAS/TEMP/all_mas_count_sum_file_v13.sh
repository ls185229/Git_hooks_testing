...
# $Id: all_mas_count_sum_file_v13.sh 3016 2015-06-25 19:27:17Z jkruse $
...
if [[ $INSTITUTION_ID = "ALL" ]]; then
    inst_list="101 105 106 107 112 113 811"
else
...
for inst_id in $inst_list
    ...
    # cnt_list="AUTH AUTH2 AUTHDB ACH VAU RCYL AVS AVSAUTH CVV2 BIN BTCH MBU PROGRAM_BLOCK"
    cnt_list="AUTH AUTHDB ACH VAU RCYL AVS AVSAUTH CVV2 BIN BTCH MBU PROGRAM_BLOCK"
    case $inst_id in
    ...
    for cnt_type in $cnt_list
        ...
        print "Script $locpth/$codename -i$inst_id -c$cnt_type $VERBOSE $TEST $YEAR_OPT $MONTH_OPT started."
        if $locpth/$codename -i$inst_id -c$cnt_type $VERBOSE $TEST $YEAR_OPT $MONTH_OPT >> $locpth/LOG/$logname; then
            print "Script $locpth/$codename -i$inst_id -c$cnt_type $VERBOSE $TEST $YEAR_OPT $MONTH_OPT completed." >> $locpth/LOG/$logname
