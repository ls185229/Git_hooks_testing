#!/usr/bin/env tclsh
# ###############################################################################
# $Id: mas_reversal.tcl 4614 2018-06-15 18:33:40Z skumar $
# $Rev: 4614 $
# ###############################################################################
#
# File Name:  mas_reversal.tcl
#
...
#    Example - mas_reversal.tcl -i 107 -d 20161026
#
#    Return - 0 = Success
#             1 = Fail
#
...
proc get_sql {} {
    ...
    set get_ent "
        SELECT 
            mtl.INSTITUTION_ID               SRC_INST_ID,
            mtl.ENTITY_ID                    MERCH_ID,
            ae.ENTITY_DBA_NAME               MERCH_NAME,
            nvl(mtl.amt_original*100,0)      AMT_TRANS,
            nvl(mtl.CURR_CD_ORIGINAL,0)      TRANS_CCD,
            mtl.CARD_SCHEME                           ,
            mtl.TRANS_REF_DATA               ARN,
            CASE 
            when mtl.tid = '010003005141' then '010003005143'
            when mtl.tid = '010003005143' then '010003005141'
            END   tid,
            mtl.TRANS_SEQ_NBR                         ,
            nvl(mtl.MAS_CODE,'MAS001')       MAS_CODE
        FROM MAS_TRANS_LOG mtl
        JOIN MAS_FILE_LOG mfl
            ON mtl.FILE_ID         = mfl.FILE_ID
            AND mtl.INSTITUTION_ID = mfl.INSTITUTION_ID
        join acq_entity ae
            ON mtl.INSTITUTION_ID = ae.INSTITUTION_ID
            AND mtl.entity_id = ae.entity_id
        WHERE 
        trunc(GL_DATE)         = '12-JUN-18'
        AND mtl.INSTITUTION_ID = '$inst_id'
        AND CARD_SCHEME        = '05'
        AND mfl.FILE_NAME LIKE '%CONV%'
        AND mtl.TRANS_REF_DATA IN
          (SELECT DISTINCT TRANS_REF_DATA
          FROM MAS_TRANS_LOG
          WHERE INSTITUTION_ID = '$inst_id'
          AND FILE_ID         IN ('1779085', '1779086', '1779087', '1779088', '1779094', '1779095', '1779096', '1779097', '1779098', '1779099', '1779100', '1779101', '1779102', '1779103', '1779104', '1779105', '1779106', '1779107', '1779108')
          AND CARD_SCHEME      = '05'
          )
...
proc get_missed_txn_sql {} {
    global get_ent inst_id txn_list
    set get_ent "
        SELECT 
            mtl.INSTITUTION_ID               SRC_INST_ID,
            mtl.ENTITY_ID                    MERCH_ID,
        ...
        join acq_entity ae
            ON mtl.INSTITUTION_ID = ae.INSTITUTION_ID
            AND mtl.entity_id = ae.entity_id
        WHERE mtl.INSTITUTION_ID = '$inst_id'
        AND mtl.TRANS_SEQ_NBR IN ($txn_list)
          and mtl.trans_sub_seq = 0
        ORDER BY  mtl.ENTITY_ID"
};
# ######################################################################################################################
