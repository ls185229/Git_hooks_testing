...
##############################################################################
#
# File Name:  mas_file_from_clearing.sh
#
...
usage(){
    print "Usage:mas_file_from_clearing.sh [-i institution_id] [-d date[YYYYMMDD]]"
    print "      Eg: 1) mas_file_from_clearing.sh -i 101" 
    print "          2) mas_file_from_clearing.sh -i 101 -d 20160919"
}
...
######## MAIN ####################################
print "Beginning Reversal MAS file for $inst_id for date $run_date" >> $locpth/INST_$inst_id/LOG/mas_file_from_clearing.log

code_nm="$locpth/mas_file_from_clearing.tcl -i $inst_id -d $run_date $file_name $VERBOSE $test"

echo `date +%Y/%m/%d\ %T` command $0 "$@"

if $code_nm; then
    print "Ending Reversal MAS file for $inst_id for date $run_date" >> $locpth/INST_$inst_id/LOG/mas_file_from_clearing.log
else
    echo "`date +%Y%m%d%H%M%S` $0 $inst_id Evaluated \n$sysinfo\nEngineer, see $locpth/INST_$inst_id/LOG/mas_file_from_clearing.log" | 
