...
# $Id: new_mas_chgbck_file.tcl 3577 2015-11-13 18:15:11Z bjones $
...
set get_ent "select idm.tid, epl.* from
ep_event_log epl
join in_draft_main idm on idm.trans_seq_nbr = epl.trans_seq_nbr
where (ems_item_type in ('CB1','CB2','RR1','RR2') or (ems_item_type in ('CR2') and epl.card_scheme = '08')) and event_log_date like '$e_date%' and event_status in ('NOTIFY', 'NEW')"
puts $get_ent
...
if { [catch {
            while {[set loop [orafetch $clr_get1 -dataarray a -indexbyname]] == 0} {
                ...
                if {$oerr != 1403} {
                    ...
                    set get_sale1 "select * from in_draft_main where ARN = '$a(ARN)' and trans_seq_nbr = $a(TRANS_SEQ_NBR)"
                    puts $get_sale1
                    ...
                    switch $a(EMS_ITEM_TYPE) {
                        "CB1"   {
                            switch $c(TID) {
                                ...
                                "010103015102" {
                                    set mas_tid "010003015102"
                                }
                                default {
                                    error "unknown TID $c(TID)"
                                }
                            }
                            switch $b(CARD_SCHEME) {
                                ...
                                default {
                                    error "unsupported card scheme: $b(CARD_SCHEME)"
                                }
                            }
                        ...
                        "CR2" {
                            switch $b(CARD_SCHEME) {
                                "08" {
                                    ...
                                    switch $a(TID) {
                                        "010103005301" {
                                            set mas_tid "010003005301"
                                        }
                                        "010103005302" {
                                            set mas_tid "010003005302"
                                        }
                                        default {
                    ...
                    if {$a(TRANS_CCD) == "840"} {
                        ...
                        set get_ocb "select * from in_draft_main where trans_seq_nbr = '$a(TRANS_SEQ_NBR)'"
                        orasql $clr_get21 $get_ocb

                        orafetch $clr_get21 -dataarray ocb -indexbyname

                        set rec(amt_original) $ocb(AMT_RECON)

                        write_md_record $fileid rec

                        set totcnt [expr $totcnt + 1]
                        set totamt [expr $totamt + $ocb(AMT_RECON)]

                        set ftotcnt [expr $ftotcnt + 1]
                        set ftotamt [expr $ftotamt + $ocb(AMT_RECON)]
                    }
...
if {$loop == 1403 && $chk == 0} {
    ...
    set result1 ""
    catch {exec cp -p $outfile /clearing/filemgr/MAS/MAS_FILES/$outfile} result1
}
if {$result1 == ""} {
