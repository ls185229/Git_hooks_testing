...
# $Id: all_mas_txn_count_sum_file_v13.tcl 4372 2017-09-29 16:43:20Z skumar $
...
global clr_db_userid clr_db_pwd auth_db_userid auth_db_pwd clrdb authdb
set box $env(SYS_BOX)
...
set clr_db_userid $env(IST_DB_USERNAME)
set clr_db_pwd $env(IST_DB_PASSWORD)
...
set id_var  "$Id: all_mas_txn_count_sum_file_v13.tcl 4372 2017-09-29 16:43:20Z skumar $nihil"
set rev_var "$Rev: 4372 $nihil"
...
proc usage { f_type } {
    ...
    puts stdout "\n Valid Count File Type values are:
        ...
        18) VBV          -- Credit Card VBV transaction count file
        19) VLCK         -- Velocity Check transaction count file
        20) VZEROFORCE   -- Visa Zero Dollar Floor Limit Fee File
        21) DBMONTHLY    -- PIN Debit Monthly Network fee
        "
}
...
proc get_inst {} {
    ...
    global arr_cfg inst_id s_name fname dot cnt_type run_mnth run_year auth_get clr_get clr_get1 clr_get2 port_get
    ...
    switch $f_type {
        ...
        "AUTHDB" {
            set nm "select
                         ...
                         case when t.network_id = '29' then 'Y' end is_ebt
                         --,
                         --aa.ebt_type,
                         --t.network_id
                    ...
                    group by
                         ...
                         case when t.network_id = '29' then 'Y' end
                         --,
                         --aa.ebt_type,
                         --t.network_id
                    order by t.mid"
        ...
        "ASSOC" {
            set nm "select  /*+INDEX(t TRANSACTION_AUTHDATETIME)*/
                    t.mid,
                    count(t.amount) as cnt,
                    sum(case when t.action_code = '000' then t.amount else 0 end) as amt,
                    sum(t.amount) as amt_sum,
                    t.card_type,
                    t.request_type,
                    decode (t.action_code, '000', 'A', 'D') as status,
                    case when t.request_type = '0250'
                        and t.other_data3 = t.other_data4
                        then 1 else 0 end od3_eq_od4,
                    --b.acct_fund_src
                        'C' acct_fund_src,
                    /*
                    case when not (
                            b.CNTRY_CODE_NUM    = '840'
                        or  b.CNTRY_CODE_ALPHA2 = 'US'
                        or (
                            b.CNTRY_CODE_NUM    is null
                            and b.CNTRY_CODE_ALPHA2 is null
                        )
                    ) then 'I'  end
                    */
                    null international
                ...
                */
                    ...
                    case when t.request_type = '0250' and t.other_data3 = t.other_data4 then 1 else 0 end
                    /*,
                    )
                    group by entity_id,
                    */
...
proc connect_to_dbs {} {
    ...
    global clr_db_userid clr_db_pwd auth_db_userid auth_db_pwd
    ...
    if {[catch {set db_login_handle [oralogon $clr_db_userid/$clr_db_pwd@$clrdb]} result]} {
...
MASCLR::log_message "mas_code list from mas_code_grp s: $s" 3
...
MASCLR::log_message "In start of main" 3
...
if {$cnt_type == "AUTH"  || $cnt_type == "WOOT"      || $cnt_type == "WOOT2" ||
    ...
    $cnt_type == "FORCE" || $cnt_type == "DBMONTHLY" ||
    $cnt_type == "AU"   || $cnt_type == "ASSOC"  } {

    ##### Getting mid list from entity_to_auth.
    MASCLR::log_message "Getting mid list from entity_to_auth" 3
    ...
    if {$cnt_type == "WOOT" || $cnt_type == "WOOT2" } {
        ...
        while {[set x1 [orafetch $clr_get1 -dataarray e2a -indexbyname]] == 0} {
            if {$mid_count == 0} {
                MASCLR::log_message "appending '$e2a(MID)' to $list_name" 4
    ...
    MASCLR::log_message "Getting mid list from merchant_flag via entity_to_auth" 3
...
MASCLR::log_message "before midlist list of mid_lists loop " 3
...
foreach midlist $list_of_mid_lists {
    ...
    MASCLR::log_message "using $outfile for midlist $midlist"
    ...
    MASCLR::log_message "Before Loop to fetch records " 3
    ...
    while {[set loop [orafetch $cursor -dataarray a -indexbyname]] == 0} {
        ...
        set rec(card_scheme) [string range $c_sch 0 1]
        if { $cnt_type == "AUTHDB" } {
            MASCLR::log_message "card_type: $a(CARD_TYPE), card_scheme  $c_sch, $rec(card_scheme), $a(IS_EBT)"  3
        } else {
            MASCLR::log_message "card_type: $a(CARD_TYPE), card_scheme  $c_sch, $rec(card_scheme)"  3
        }
        ...
        if { $cnt_type == "AU" } {
            ...
            if {[set mcdchk $mas_cd($c_sch-$a(REQUEST_TYPE)-$a(STATUS))] != "IGNORE"} {
                ...
                # if { $cnt_type == "ASSOC" } {
                #     append rec(mas_code) $a(ACCT_FUND_SRC) $a(INTERNATIONAL)
                # }
                if { $cnt_type == "AUTHDB" } {
                    if { $a(IS_EBT) == "Y" } {
                        set tmp [string range $rec(mas_code) 0 1]
                        append tmp "E"
                        append tmp [string range $rec(mas_code) 2 99]
                        set rec(mas_code) $tmp
                    }
                }
            } else {
                continue
    ...
    MASCLR::log_message "End of Loop to fetch records " 3
