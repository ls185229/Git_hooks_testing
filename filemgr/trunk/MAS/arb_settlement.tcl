...
# $Id: arb_settlement.tcl 3590 2015-11-23 18:39:38Z bjones $
...
proc main {} {
    ...
    CASE WHEN epl.trans_seq_nbr_orig = 0 THEN (SELECT MAX(trans_seq_nbr) FROM in_draft_main WHERE pan              = idm.pan
     ...
     (SELECT merch_id FROM in_draft_main WHERE trans_seq_nbr =
      ...
      idm.recon_exp                   , idm.msg_type                    , NVL(idm.function_cd,0) as function_cd ,
...
in the amount of [expr $arbitration_transaction_arr(AMT_RECON) / 100.0]\n\n"
    ...
    if { $matches_found > 0 } {
        ...
        set email(mailto) "clearing@jetpay.com,cblizzard@tsys.com,merlinclearing@tsys.com,chargebacks@jetpay.com"
        set email(mailfrom) ""
