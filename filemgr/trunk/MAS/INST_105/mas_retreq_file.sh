...
print "Beginning  MAS FILE for JETPAY at `date +%Y%m%d%H%M%S`" >> $locpth/LOG/retreq.log

if $locpth/mas_retrval_file.tcl $sht_name $inst_id `date +%Y%m%d` >> $locpth/LOG/retreq.log; then
	print "Script $locpth/mas_retrval_file.tcl completed sucessfully" | tee -a $locpth/LOG/retreq.log
	print "Ending successfully at `date +%Y%m%d%H%M%S`" >> $locpth/LOG/retreq.log
else 
        print "FAILED at `date +%Y%m%d%H%M%S`" >> $locpth/LOG/retreq.log
        echo "SCRIPT: $locpth/mas_retrval_file.tcl (Failed)\nLOG: $locpth/LOG/retreq.log\n ERROR: \n `tail -20 $locpth/LOG/retreq.log`" | mailx -r $MAIL_FROM -s "$msubj_c" $MAIL_TO
fi
