...
################################################################################
#
# File Name:  send_avcvbb_clr2mas.tcl
#
# Description:  This program sends a MAS AVS/CVV and BIN blocking transaction
#               count summary file to MAS for the
#               institution.
#
...
if {$chklpyear == "0228"} {
    ...
    if {$lp(LEAP_YR_DT) == "0229"} {
        puts "leap year exiting file run. This will be ran on tomorrow at Feb 29"
        set mbody "leap year exiting file run. This will be ran on tomorrow at Feb 29"
...
if {[file exists /clearing/filemgr/process.stop]} {
    exec echo "PREVIOUS PROCESS NOT COMPLETED ALL OTHER PROCESSES STOPED : Msg sent from Counting Script send_avcvbb_file.tcl" | mailx -r clearing@jetpay.com -s "PROD: MAS: URGENT!!! CALL SOMEONE :: PREVIOUS PROCESS NOT COMPLETED" clearing@jetpay.com assist@jetpay.com
    exit 1
} else {
...
catch {set y [exec find $env(PWD)/MAS_FILES -mtime 0 | grep $inst_id.AVSCVVBN.01*]} result
...
puts $y
set infilenm [string range [lindex $y 0] 41 end]
puts $infilenm

exec chmod 775 $env(PWD)/$infilenm
set dummy ""
set dummy [exec cp -p $env(PWD)/$infilenm $maspath/data/in]
puts $dummy
...
set err [exec mv MAS_FILES/$infilenm ARCHIVE]
