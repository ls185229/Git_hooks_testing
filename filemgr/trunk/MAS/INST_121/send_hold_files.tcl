...
################################################################################
#
# File Name:  send_hold_files.tcl
#
# Description:  This program sends a MAS hold file to MAS for the institution.
#
...
catch {set y [exec find $env(PWD)/HOLD_FILES -mtime 0 | grep $inst_id.*]} result
...
puts $y
set counting [string range [lindex $y 0] 41 end]
puts $counting

exec chmod 775 $env(PWD)/$counting
set dummy ""
set dummy [exec cp -p $env(PWD)/$counting $maspath/data/in]
puts $dummy
...
set err [exec mv HOLD_FILES/$counting ARCHIVE]
