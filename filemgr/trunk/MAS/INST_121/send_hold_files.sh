...
################################################################################
#
# File Name:  send_hold_files.sh
#
# Description:  This program initiations sending of a MAS hold file to 
...
if send_hold_files.tcl >> $locpth/LOG/xfercnt.log; then
	print "Script send_hold_files.tcl completed." | tee -a $locpth/LOG/xfercnt.log
else
	echo "Script send_hold_files.tcl for $inst_id failed" | mailx -r $MAIL_FROM -s "$SYS_BOX: MAS: $locpth/send_hold_files.tcl Failed" $MAIL_TO
fi
