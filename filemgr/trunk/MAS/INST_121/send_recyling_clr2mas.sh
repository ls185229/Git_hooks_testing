...
################################################################################
#
# File Name:  send_recyling_clr2mas.sh
#
# Description:  This program initiations sending of a MAS recycling file to 
#               MAS for the institution.
#              
...
print "Beginning  CLEARING FILE TRANSFER at `date +%Y%m%d%H%M%S`" | tee -a $locpth/LOG/xferrecyl.log

if send_recyling_clr2mas.tcl $inst_id >> $locpth/LOG/xferrecyl.log; then
	print "Script send_recyling_clr2mas.tcl completed" | tee -a $locpth/LOG/xferrecyl.log
else
	echo "Script $locpth/send_recyling_clr2mas.tcl for $inst_id failed" | mailx -r $MAIL_FROM -s "$SYS_BOX : MAS: send_recyling_clr2mas.tcl Failed" $MAIL_TO
	exit 1
fi

print "Ending CLEARING FILE TRANSFER at `date +%Y%m%d%H%M%S`" | tee -a $locpth/LOG/xferrecyl.log
