...
################################################################################
#
# File Name:  mas_recyl_txn_count_sum_file_v13.tcl
#
# Description:  This program initiations the generation of a MAS transaction 
#               count summary file for the institution.
#              
...
set get_ent_mid "select mid from entity_to_auth where institution_id = '$inst_id' and FILE_GROUP_ID = '$s_name' and STATUS = 'A'"
...
while {[set x1 [orafetch $clr_get1 -dataarray e2a -indexbyname]] == 0} {
	...
	if {$mcnt >= 1000} {
		Puts "We have more that 1000 MID count file failed."
		exit 1
	}
}

if {$midlist == ""} {
	puts "No merchants found with institution_id = '$inst_id' and FILE_GROUP_ID = '$s_name' and STATUS = 'A'."
	exec rm $outfile
...
set get_ent "select mid, visa_id, mastercard_id, currency_code from $tblnm_mr where mid in ($midlist)"
...
###### 'D') ond rder by mid  
set get_info "select mid, count(amount) as cnt, sum(amount) as amt, card_type, request_type, decode (action_code,'000','A','D') as status from $tblnm_tn where mid in ($midlist) and authdatetime >= $begintime || '$begint' and authdatetime < $endtime || '$endt' group by mid, card_type, request_type, decode (action_code,'000','A','D') order by mid"
