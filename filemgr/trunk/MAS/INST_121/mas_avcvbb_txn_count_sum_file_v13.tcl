...
################################################################################
#
# File Name:  mas_avcvbb_txn_count_sum_file_v13.tcl
#
...
set clr_get1 [oraopen $db_logon_handle]
...
#setting tid lists
set mas_tid(BIN) "010070010002"
set mas_tid(AVS) "010070010003"
set mas_tid(CVV) "010070010004"
...
set get_mids "select * from MERCHANT_FEE_FLAGS where STATUS = 'A' and (AVS_REJECT = 'Y' or BIN_BLOCKING = 'Y' or CVV2_REJECT = 'Y') and institution_id = '$inst_id'"
...
while {[set z [orafetch $clr_get1 -dataarray  mid1 -indexbyname]] == 0} {
	...
	if {$mid1(AVS_REJECT) == "Y"} {
		set flgchk($mid1(MID)-$avs) "Y"
	} else {
		set flgchk($mid1(MID)-$avs) "N"
	}

	if {$mid1(BIN_BLOCKING) == "Y"} {
		set flgchk($mid1(MID)-$binb) "Y"
	} else {
		set flgchk($mid1(MID)-$binb) "N"
	}

	if {$mid1(CVV2_REJECT) == "Y"} {
		set flgchk($mid1(MID)-$cvv) "Y"
	} else {
		set flgchk($mid1(MID)-$cvv) "N"
	}
}
...
decode (action_code, '930',  'AVS', '931', 'AVS', '932', 'AVS', '981',  'AVS','099', 'BIN', '933', 'CVV') as dtype \
from $tblnm_tn \
where \
mid in ($mid_list) \
and authdatetime >= $begintime || '$begint' \
and authdatetime < $endtime || '$endt' \
and action_code in ( '930', '931', '932', '981', '099', '933') \
group by \
mid, \
card_type, \
request_type, \
decode (action_code,'000','A','D'), \
decode (action_code, '930',  'AVS', '931', 'AVS', '932', 'AVS', '981',  'AVS','099', 'BIN', '933', 'CVV') \
order by \
mid, \
card_type, \
request_type, \
...
while {[set loop [orafetch $auth_get -dataarray a -indexbyname]] == 0} {
	...
	puts "AVS $mid1(MID) $flgchk($mid1(MID)-$avs)"
	
	###### Writing Massage Detail records
	set rec(trans_id) $mas_tid($a(DTYPE))
