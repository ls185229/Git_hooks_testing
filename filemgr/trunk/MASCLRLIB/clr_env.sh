...
# $Id: clr_env.sh 3567 2015-10-27 21:08:57Z bjones $
...
if [[ "$SYS_BOX" == *QA ]]
then
    HOST_DB_MASCLR_REPORTER=TRANSQA
    HOST_DB_MASCLR_UPDATER=TRANSQA
    HOST_DB_AUTH_REPORTER=TRANSQA
    HOST_DB_AUTH_UPDATER=TRANSQA
    TEST_MODE=QA
...
then
    ...
    HOST_DB_MASCLR_UPDATER="$CLR4_DB"
        HOST_DB_AUTH_REPORTER="$RPT_DB"
        HOST_DB_AUTH_UPDATER="$ATH_DB"
    TEST_MODE=PROD
    MAIL_TO_FAILURE="clearing@jetpay.com assist@jetpay.com"
    MAIL_TO_NOTIFY="clearing@jetpay.com"
    echo `date "+%m/%d/%y %H:%M:%S"` "Setting TWO_TASK to $CLR4_DB"
...
process_stop_file="/clearing/filemgr/process.stop"
...
# output is written to stdout
    ...
    then
        ...
        log_message "$script_name" >> $process_stop_file
    fi
    ###
}   # create_process_stop
    ###
...
#   $2 -- log file to redirect that scripts output to
#
