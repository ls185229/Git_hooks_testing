...
################################################################################
# $Id: incoming_transaction_inst_fix.tcl 2552 2014-02-25 16:41:05Z msanders $
# $Rev: 2552 $
################################################################################
#
#    File Name   - incoming_transaction_inst_fix.tcl
#
#    Description - sets the instution id and other fields for transactions from the association
#      To the value of the original transaction
#
################################################################################
...
proc readCfgFile {} {
  ...
  while { [set line [gets $file_ptr]] != {}} {
    ...
    switch -exact -- [lindex  $line_parms 0] {
       ...
       "EXCLUDE_TIDS" {
          set excluded_tid_list [lindex $line_parms 1]
       }
       default {
...
puts "incoming_transaction_inst_fix.tcl Started"
...
where in_draft_main.card_scheme = card_scheme.card_scheme AND
  ...
  in_draft_main.in_file_nbr in (select in_file_nbr from in_file_log where trunc(incoming_dt) >= '$begin_date' and substr(external_file_name,1,6) in ('INMC.T','INVS.P')) AND
  in_draft_main.tid not in ($excluded_tid_list)
...
while {[orafetch $mas_sql -dataarray mas_query -indexbyname] != 1403 } {
  set Query_sql "select trans_seq_nbr, 
    ...
    AND sec_dest_inst_id is not null
  ...
  if {[orafetch $mas_sql1 -dataarray mas_query1 -indexbyname] != 1403 } {
    ...
    append body "Could not find matching trans for $mas_query(TRANS_SEQ_NBR).  Look in in_draft_main to find the transaction"
  }

}
if {$body != " "} {
  exec echo $body | mutt -s "incoming_transaction_inst_fix.tcl: Problem with fixing response transactions" clearing@jetpay.com
}
