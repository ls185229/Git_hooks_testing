...
################################################################################
# $Id: reject_reprocess_report.sh 3681 2016-02-19 22:17:42Z bjones $
# $Rev: 3681 $
################################################################################
#
#    File Name   -  reject_reprocess_report.sh
#
...
#                   given date.
#
#    Usage       -  reject_reprocess_report.sh -D yyyymmdd -F config_file.cfg
#
#                   e.g.   reject_reprocess_report.sh -d 2015032 -f test.cfg
#
################################################################################
...
init()
{
   ...
   do
     case $option in
       ...
       f) cfgFile="$OPTARG" ;;
       ?) usage
          tail -40 $logFile | mutt -s "Reject Reprocess Report failed" "$mail_err"
          exit 1;;
     esac
   done
...
if [ $? -eq 0 ]; then
   echo " Reject Reprocess Report ran successfuly on `date +%Y/%m/%d@%H:%M:%S`" >> $logFile
   echo "*************************************" >> $logFile
else
   echo "\n Reject Reprocess Report run FAILED on `date +%Y/%m/%d@%H:%M:%S`" >> $logFile
   echo "*************************************" >> $logFile
   tail -40 $logFile | mutt -s "Reject Reprocess Report failed" "$mail_err"
   exit 1
fi
