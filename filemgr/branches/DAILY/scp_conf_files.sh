...
echo "`date +%Y%m%d%H%M%S` - Beginning $cmd" | tee -a /clearing/filemgr/JOURNALS/DAILY/LOG/scp_conf_files.log

if tclsh $cmd | tee -a /clearing/filemgr/JOURNALS/DAILY/LOG/scp_conf_files.log; then
    echo "`date +%Y%m%d%H%M%S` - $cmd successful\n" | tee -a /clearing/filemgr/JOURNALS/DAILY/LOG/scp_conf_files.log
else
    echo "`date +%Y%m%d%H%M%S` - $cmd run FAILED" | tee -a /clearing/filemgr/JOURNALS/DAILY/LOG/scp_conf_files.log
