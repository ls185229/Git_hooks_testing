   ...
   default {
    ...
    set entity_qry  "
       SELECT '$sql_inst' sql_inst, '$filedate' filedate, ae.entity_id, ae.entity_dba_name, ae.entity_name,
           ae.institution_id, ae.billing_type, ae.default_mcc, NVL(aea_ma.prov_state_abbrev, NULL) as PROV_STATE_ABBREV, i.ISO_ID
       ...
       LEFT OUTER JOIN core.merchant@boardqa_trnclr1 m ON e2a.MID = m.MID

       LEFT OUTER JOIN core.MASTER_MERCHANT@boardqa_trnclr1 mm ON m.MMID = mm.MMID

       LEFT OUTER JOIN CORE.CORPORATE@boardqa_trnclr1 c ON c.CP_ID = mm.CP_ID

       LEFT OUTER JOIN core.iso@boardqa_trnclr1 i ON c.ISO_ID = i.ISO_ID
    ...
    while {[orafetch $cursor_C -dataarray row -indexbyname] != 1403} {
        ...
        dict set entity_dictionary $unique AGENT_ID     $row(ISO_ID)
