...
################################################################################
# $Id: iso_billing.sh 4871 2019-07-03 14:19:18Z lmendis $
# $Rev: 4871 $
################################################################################
#
...
usage()
{
   echo "Usage: $programName.sh [-i institution] [-d Date optional] [-E Email optional] [-f Config file]"
...
init()
{
   verboseArg=""
   testmodeArg=""
   # Arguments from tcl script. getopt $argv "d:f:hi:m:E:tv" opt optarg
   while getopts "i:d:f:E:tv" option
   do
     case $option in
       i) instId="$OPTARG" ;;
       d) runDate="$OPTARG" ;;
   ...
   if [ -n "$email_arg" ]
   then
      progArgs="$progArgs -E $email_arg"
   fi
   
   if [ -n "$mode" ]
   then
      progArgs="$progArgs -f $mode"
   fi
