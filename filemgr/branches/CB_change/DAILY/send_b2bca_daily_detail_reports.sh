...
################################################################################
# $Id: send_b2bca_daily_detail_reports.sh 4054 2017-02-06 23:16:14Z millig $
# $Rev: 4054 $
################################################################################
#
...
init_script()
{
   ...
   log_file=$locpth/LOG/send_b2bca_daily_detail_reports.log
   dd_report_mask=130.Daily_Detail_Report.$run_date*
   echo $dd_report_mask
...
#-- Upload to B2Billig
#-- 
echo "Attempting to upload Daily Detail report to B2Billig at `date +%H`:`date +%M`:`date +%S`"
...
then
    alert_someone "Daily Detail report file transfer to B2Billig $0 processing for run date $file_run_date failed."
fi
