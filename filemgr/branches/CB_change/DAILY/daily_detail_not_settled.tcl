...
proc do_report {inst} {
    ...
    global curdate requested_date today tomorrow filedate
    global daily_cursor cur_file sql_inst
...
#===============================================================================
    ...
    set sales_str "
        SELECT nvl(sum(case when(ind.tid in ('010103005101') and card_scheme = '04')
               ...
               nvl(sum(case when (ind.card_scheme = '04' and t.tid in ($reptid) and t.tid_settl_method = 'D')
                            then 1 else 0 end), 0)                                                                as VREP_C_CNT,
               nvl(sum(case when (ind.card_scheme = '04' and t.tid in ($reptid) and t.tid_settl_method = 'D')
                            then nvl(ind.amt_trans/100, 0) else 0 end), 0)                                                as VREP_C_AMT,
               nvl(sum(case when (ind.card_scheme = '04' and t.tid in ($reptid) and t.tid_settl_method = 'C')
                            then 1 else 0 end), 0)                                                                as VREP_R_CNT,
               nvl(sum(case when (ind.card_scheme = '04' and t.tid in ($reptid) and t.tid_settl_method = 'C')
                        then nvl(ind.amt_trans/-100, 0) else 0 end), 0)                                              as VREP_R_AMT,

               nvl(sum(case when (ind.card_scheme = '05' and t.tid in ($reptid) and t.tid_settl_method = 'D')
                            then 1 else 0 end), 0)                                                                as MREP_C_CNT,
               nvl(sum(case when (ind.card_scheme = '05' and t.tid in ($reptid) and t.tid_settl_method = 'D')
                            then nvl(ind.amt_trans/100, 0) else 0 end), 0)                                                as MREP_C_AMT,
               nvl(sum(case when (ind.card_scheme = '05' and t.tid in ($reptid) and t.tid_settl_method = 'C')
                            then 1 else 0 end), 0)                                                                as MREP_R_CNT,
               nvl(sum(case when (ind.card_scheme = '05' and t.tid in ($reptid) and t.tid_settl_method = 'C')
                        then nvl(ind.amt_trans/-100, 0) else 0 end), 0)                                               as MREP_R_AMT,

               nvl(sum(case when (ind.card_scheme = '03' and t.tid in ($reptid) and t.tid_settl_method = 'D')
                            then 1 else 0 end), 0)                                                                as XREP_C_CNT,
               nvl(sum(case when (ind.card_scheme = '03' and t.tid in ($reptid) and t.tid_settl_method = 'D')
                            then nvl(ind.amt_trans/100, 0) else 0 end), 0)                                                as XREP_C_AMT,
               nvl(sum(case when (ind.card_scheme = '03' and t.tid in ($reptid) and t.tid_settl_method = 'C')
                            then 1 else 0 end), 0)                                                                as XREP_R_CNT,
               nvl(sum(case when (ind.card_scheme = '03' and t.tid in ($reptid) and t.tid_settl_method = 'C')
                        then nvl(ind.amt_trans/-100, 0) else 0 end), 0)                                               as XREP_R_AMT,

               nvl(sum(case when (ind.card_scheme = '08' and t.tid in ($reptid) and t.tid_settl_method = 'D')
                            then 1 else 0 end), 0)                                                                as DREP_C_CNT,
               nvl(sum(case when (ind.card_scheme = '08' and t.tid in ($reptid) and t.tid_settl_method = 'D')
                            then nvl(ind.amt_trans/100, 0) else 0 end), 0)                                                as DREP_C_AMT,
               nvl(sum(case when (ind.card_scheme = '08' and t.tid in ($reptid) and t.tid_settl_method = 'C')
        ...
        WHERE ind.in_file_nbr = ifl.in_file_nbr
          AND ind.tid = t.tid
          AND ifl.end_dt >= '$today' and ifl.end_dt < '$tomorrow'
...
#   puts "vs_inchg is done at [clock format [clock seconds]]"
    ...
    set mc_inchg "
        ...
        where ipm.RECON_MSG_SEQ_NBR= r.RECON_MSG_SEQ_NBR
          ...
          and i.end_dt >= '$today' and i.end_dt < '$tomorrow'
...
##========  REPROCESSED REJECTS ==========
    ...
    set query_repro "
        ...
        where SUBSTR(tid,1,11) = '01010300510'
          ...
          and activity_dt >= '$today' and activity_dt < '$tomorrow'
          and SEC_DEST_INST_ID = '$sql_inst'
          and in_file_nbr in (select in_file_nbr from in_file_log
                              where end_dt >= '$today' and end_dt < '$tomorrow')
